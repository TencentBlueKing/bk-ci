...
unreg_systemd_ci (){
  ...
  sd_target="bk-ci.target"  # systemd target名.
  ...
  # 判断是否需要删除target.
  shopt -s nullglob
  a_sd_service_files=( "$systemd_unit_dir/bk-ci-"*.service )
  shopt -u nullglob
  if [ ${#a_sd_service_files[@]} -eq 0 ]; then
    echo "no ci service remaining, disable target $sd_target."
...
# 检查脚本用法.
if [ $# -lt 1 ]; then
  echo >&2 "Usage: $0 proj...   -- unregister systemd service for bk-ci projects."
  exit 1
fi
# 检查systemd
