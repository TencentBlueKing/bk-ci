...
prestart_ci_agentless (){
  prestart_ci_dockerhost || return $?
  # TODO 适配自定义TAG.
  WAIT_TIMEOUT=180 wait_dns_up "dispatch-devops.service.consul" || return 4
}
prestart_ci_dockerhost (){
  # dockerhost需要提供agent-package/script/供私有构建机使用.
  tip_dir_exist "../agent-package/" || return 3
  # TODO 适配自定义TAG.
  wait_dns_up "bk-ci.service.consul" || return 4
}

# artifactory
prestart_ci_artifactory (){
  # 发现多节点部署?
  # 检查服务发现地址? 如果存在且非自己, 则检查数据目录为共享文件系统?
  #check_empty_var BK_CI_CONSUL_HTTP_PORT BK_CI_CONSUL_DISCOVERY_TAG || return 1
  #local check_url="http://127.0.0.1:$BK_CI_CONSUL_HTTP_PORT/v1/health/checks/artifactory-$BK_CI_CONSUL_DISCOVERY_TAG"
...
prestart_ci_assembly (){
  tip_dir_exist "../agent-package/" || return 3
}

prestart_ci_environment (){
...
prestart_func=prestart_ci_$MS_NAME_WORD
