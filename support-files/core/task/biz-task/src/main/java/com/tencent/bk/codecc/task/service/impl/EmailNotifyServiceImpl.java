...
public class EmailNotifyServiceImpl implements EmailNotifyService {
    ...
    public void sendRtx(RtxNotifyModel rtxNotifyModel) {
        try{
            ...
            CodeCCResult<List<ToolLastAnalysisResultVO>> toolLastAnalysisResult = client.get(ServiceTaskLogRestResource.class).
            ...
            if(rtxNotifyModel.getSuccess())
            {
                ...
                List<ToolLastAnalysisResultVO> toolLastAnalysisResultVOList = toolLastAnalysisResult.getData();
    ...
    private void toolsDataInfo(DailyDataReportReqModel dailyDataReportReqModel, Long taskId, Set<String> toolNames)
    {
        ...
        CodeCCResult<JSONArray> dataReportResult = client.get(ServiceDefectTreeResource.class).getBatchDataReports(taskId, toolNames);
    ...
    {
        CodeCCResult<Set<CodeRepoVO>> codeRepoResult = client.get(ServiceRepoResource.class).getCodeRepoByTaskIdAndBuildId(taskId, null);
        if(codeRepoResult.isNotOk() || CollectionUtils.isEmpty(codeRepoResult.getData())){
            log.error("get repo url fail! task id: {}", taskId);
            return;
        }
        Set<CodeRepoVO> codeRepoVOS = codeRepoResult.getData();
