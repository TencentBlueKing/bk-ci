...
public class CheckerSetDao
{
    ...
    public void updateAllVersionCheckerSet(CheckerSetEntity checkerSetEntity)
    {
        ...
        update.set("checker_set_name", checkerSetEntity.getCheckerSetName())
                .set("scope", checkerSetEntity.getScope()).set("last_update_time", checkerSetEntity.getLastUpdateTime());
     ...
     * @return
     */
    public List<CheckerSetEntity> findByComplexCheckerSetCondition(String keyWord, Set<String> checkerSetIds, Set<String> codeLang, Set<CheckerSetCategory> checkerSetCategory,
                                                                   Set<String> tool, Set<CheckerSetSource> checkerSetSource, String creator, Boolean officialFlag, Pageable pageable, boolean needCheckerSetFilter)
    {
        Query query = new Query();
        ...
        if (CollectionUtils.isNotEmpty(codeLang))
        {
            ...
            langCriteria.add(new Criteria().andOperator(langArrayCriteria, new Criteria().orOperator(langOrCriteria.toArray(new Criteria[0]))));
        ...
        if (null != officialFlag && officialFlag)
        {
            ...
            Criteria secondSourceCriteria = Criteria.where("checker_set_source").in(Arrays.asList(CheckerSetSource.DEFAULT,
     ...
     * @return
     */
    public List<CheckerSetEntity> findMoreByCondition(String quickSearch, Set<String> codeLang, Set<CheckerSetCategory> checkerSetCategory,
            Set<String> projectCheckerSetIds, Boolean projectInstalled, Pageable pageable)
    {
        // 查询条件
        ...
        if (CollectionUtils.isNotEmpty(checkerSetCategory))
        {
            List<String> categorylist = checkerSetCategory.stream().map(category -> category.name()).collect(Collectors.toList());
        ...
        if (projectInstalled != null)
        {
            if (projectInstalled)
            {
                criteriaList.add(
                    new Criteria().orOperator(
                        Criteria.where("checker_set_source").in(Lists.newArrayList(CheckerSetSource.RECOMMEND.name(), CheckerSetSource.DEFAULT.name())),
            ...
            {
                criteriaList.add(Criteria.where("checker_set_source").nin(Lists.newArrayList(CheckerSetSource.RECOMMEND.name(), CheckerSetSource.DEFAULT.name())));
...
//            criteriaList.add(Criteria.where("enable").is(CheckerConstants.CheckerSetEnable.ENABLE.code()));
        ...
        AggregationResults<CheckerSetEntity> queryResult = mongoTemplate.aggregate(agg, "t_checker_set", CheckerSetEntity.class);
