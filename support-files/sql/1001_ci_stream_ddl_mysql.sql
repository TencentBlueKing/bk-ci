...
(
    34
) COLLATE utf8mb4_bin DEFAULT NULL COMMENT 'YAML版本号',
    `DIRECTORY` varchar
(
    512
) NOT NULL DEFAULT '.ci/' COMMENT '文件子路径',
    `LAST_UPDATE_BRANCH` varchar
(
    255
) DEFAULT NULL COMMENT '最新的文件触发分支',
    PRIMARY KEY
(
    `ID`
),
    UNIQUE KEY `PIPELINE_ID`
(
    `PIPELINE_ID`
),
