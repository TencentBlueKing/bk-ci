...
CREATE TABLE IF NOT EXISTS `T_PROJECT` (
  ...
  `tenant_english_name` varchar(32) DEFAULT NULL COMMENT '租户英文名',
...
(
    ...
    `UPDATE_TIME` datetime    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_LABEL_ID_PROJECT_ID` (`LABEL_ID`, `PROJECT_ID`),
...
(
    ...
    `BEFORE_KPI_CODE`       varchar(64)      DEFAULT NULL COMMENT '变更前KPI产品编码',
    `AFTER_KPI_CODE`        varchar(64)      DEFAULT NULL COMMENT '变更后KPI产品编码',
    `BEFORE_KPI_NAME`       varchar(128)     DEFAULT NULL COMMENT '变更前KPI产品名称',
    `AFTER_KPI_NAME`        varchar(128)     DEFAULT NULL COMMENT '变更后KPI产品名称',
...
(
    ...
    `ICOS_PRODUCT_CODE` varchar(64) default NULL COMMENT '财务ID',
    `ICOS_PRODUCT_NAME` varchar(64) default NULL COMMENT '财务名称',
    `CROS_CHECk` bit(1) DEFAULT NULL COMMENT '是否有效',
    `CREATED_AT`            timestamp   NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
CREATE TABLE IF NOT EXISTS `T_PROJECT_TAG_HISTORY` (
    `VERSION` varchar(64) NOT NULL COMMENT '发布版本',
    `CHANNEL` varchar(32) NOT NULL COMMENT '项目渠道',
    `PROJECT_ID` varchar(64) NOT NULL COMMENT '项目ID',
    `BATCH_PERCENT` int(11) NOT NULL COMMENT '迁移批次百分比',
    `SOURCE_TAG` varchar(32) NOT NULL COMMENT '源集群路由tag',
    `TARGET_TAG` varchar(32) NOT NULL COMMENT '目标集群路由tag',
    `STATUS` varchar(32) NOT NULL DEFAULT 'INIT' COMMENT '状态: INIT/PUBLISHING/PUBLISHED/ROLLBACK',
    `CREATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `UPDATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`VERSION`, `CHANNEL`, `PROJECT_ID`, `CREATE_TIME`),
    KEY `IDX_TPTH_VERSION_CHANNEL_BATCH` (`VERSION`, `CHANNEL`, `BATCH_PERCENT`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目路由历史表';
