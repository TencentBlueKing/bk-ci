...
-- ----------------------------
-- Table structure for T_LOG_BUILD_PROJECT
-- ----------------------------

CREATE TABLE IF NOT EXISTS `T_LOG_BUILD_PROJECT` (
  `BUILD_ID` varchar(64) NOT NULL COMMENT '构建ID',
  `PROJECT_ID` varchar(64) DEFAULT NULL COMMENT '项目ID，空表示尚未上报',
  `PIPELINE_ID` varchar(64) DEFAULT NULL COMMENT '流水线ID，空表示尚未上报',
  `CREATED_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `UPDATED_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`BUILD_ID`),
  KEY `IDX_PROJECT_ID` (`PROJECT_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='构建与项目/流水线映射，供日志流量聚合与查询鉴权';
