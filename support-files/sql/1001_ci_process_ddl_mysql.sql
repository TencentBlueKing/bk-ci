...
CREATE TABLE IF NOT EXISTS `T_PIPELINE_COPY_TASK_RESOURCE` (
    ...
    `AUTO_FINISH`                bit(1)       NOT NULL DEFAULT b'0' COMMENT '是否自动完成',
    `NEED_COMPLETION`            bit(1)       NOT NULL DEFAULT b'0' COMMENT '资源是否需要补齐',
    `NEED_TRANSFER`              bit(1)       NOT NULL DEFAULT b'0' COMMENT '资源是否需要迁移',
    `CONFIRMED`                  bit(1)       NOT NULL DEFAULT b'0' COMMENT '用户是否已确认处理完成',
    `CREATE_TIME`                timestamp    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `UPDATE_TIME`                timestamp    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`PROJECT_ID`, `TASK_ID`, `PIPELINE_ID`, `RESOURCE_ID`, `RESOURCE_TYPE`),
    KEY `IDX_TASK_RESOURCE` (`PROJECT_ID`, `TASK_ID`, `RESOURCE_ID`, `RESOURCE_TYPE`)
) ENGINE = InnoDB
