...
CREATE TABLE IF NOT EXISTS `T_RESOURCE_PUBLIC_VAR_GROUP_RELEASE_RECORD` (
    ...
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNQ_TPPVGRR_GROUP_PROJECT_NAME_VERSION` (`PROJECT_ID`,`GROUP_NAME`,`VERSION`),
...
CREATE TABLE IF NOT EXISTS `T_VAR_REF_DETAIL` (
   ...
   `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
   PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_TVRD_VAR_RESOURCE` (`PROJECT_ID`,`RESOURCE_TYPE`,`RESOURCE_ID`,`REFER_VERSION`,`VAR_NAME`)
