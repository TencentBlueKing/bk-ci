...
CREATE TABLE IF NOT EXISTS `T_EXTENSION_SERVICE_VERSION_LOG` (
  ...
  `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `UPDATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  `PACKAGE_SIZE` text DEFAULT NULL COMMENT '包大小',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_STORE_BELONG_DEPT_REL` (
   `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键',
   `STORE_CODE` varchar(64) NOT NULL DEFAULT '' COMMENT '组件编码',
   `STORE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT '组件类型 0：插件 1：模板 2：镜像 3：IDE 插件 4：微扩展',
   `BG_ID` varchar(11) NOT NULL COMMENT 'bgID',
   `BG_NAME` varchar(256) NOT NULL COMMENT 'bg名称',
   `DEPT_ID` varchar(11) DEFAULT NULL COMMENT '部门ID',
   `DEPT_NAME` varchar(256) DEFAULT NULL COMMENT '部门名称',
   `CENTER_ID` varchar(11) DEFAULT NULL COMMENT '中心ID',
   `CENTER_NAME` varchar(256) DEFAULT NULL COMMENT '中心名称',
   `GROUP_ID` varchar(11) DEFAULT NULL COMMENT '组ID',
   `GROUP_NAME` varchar(256) DEFAULT NULL COMMENT '组名称',
   `BUSINESS_LINE_ID` varchar(20) DEFAULT NULL COMMENT '业务线ID',
   ...
   `UPDATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (`ID`),
   UNIQUE KEY `UNI_INX_TSBDR_TYPE_CODE` (`STORE_TYPE`,`STORE_CODE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='组件所属机构关联关系表';
