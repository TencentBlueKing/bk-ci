...
CREATE TABLE IF NOT EXISTS `T_EXPERIENCE` (
  ...
  `REPO_CREATE_TIME` datetime DEFAULT NULL COMMENT '构件创建时间',
  ...
  `CLASSIFY` varchar(20) DEFAULT '' COMMENT '分类标识',
  `APP_NAME_I18N` varchar(128) NULL COMMENT 'i18n应用名',
  `ENABLE_PUBLIC_ACCESS` bit(1) NOT NULL DEFAULT b'0' COMMENT '允许基地公网访问',
  `ENABLE_DEVX_ACCESS` bit(1) NOT NULL DEFAULT b'0' COMMENT '允许合研环境访问',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_EXPERIENCE_GROUP_INNER` (
  ...
  `DEPT_FULL_NAME` varchar(100) NOT NULL DEFAULT ''  COMMENT '部门全称',
  `CREATE_TIME` datetime NOT NULL COMMENT '创建日期',
  `UPDATE_TIME` datetime NOT NULL COMMENT '更新日期',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_EXPERIENCE_GROUP_DEPARTMENT` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `GROUP_ID` bigint(20) NOT NULL COMMENT '对应的组号',
  `DEPT_ID` varchar(20) NOT NULL COMMENT '部门ID',
  `DEPT_NAME` varchar(100) NOT NULL COMMENT '部门名称',
  `DEPT_LEVEL` int(11) NOT NULL COMMENT '部门层级',
  `DEPT_FULL_NAME` varchar(100) NOT NULL COMMENT '部门全称',
  `CREATE_TIME` datetime NOT NULL COMMENT '创建日期',
  `UPDATE_TIME` datetime NOT NULL COMMENT '更新日期',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UNI_GROUPID_DEPTID` (`GROUP_ID`,`DEPT_ID`),
  KEY `IDX_DEPT_ID` (`DEPT_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='体验--体验组--公司部门';
...
CREATE TABLE IF NOT EXISTS `T_EXPERIENCE_P2P_CONNECT`
(
  `ID` BIGINT(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `RECORD_ID` BIGINT(20) NOT NULL COMMENT '体验ID',
  `SENDER` VARCHAR(64) NOT NULL COMMENT '发送方ID',
  `RECEIVER` VARCHAR(64) NOT NULL COMMENT '接收方ID',
  `CREATE_TIME` datetime NOT NULL COMMENT '创建时间',
...
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT 'P2P连接记录表';

SET FOREIGN_KEY_CHECKS = 1;

CREATE TABLE IF NOT EXISTS `T_EXPERIENCE_DOWNLOAD_SPEED` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT,
  `PROJECT_ID` varchar(64) NOT NULL COMMENT '项目id',
  `USER_ID` varchar(64) NOT NULL COMMENT '用户id',
  `ARTIFACTORY_TYPE` varchar(32) NOT NULL COMMENT '仓库类型',
  `ARTIFACTORY_PATH` mediumtext NOT NULL COMMENT '仓库路径',
  `RECORD_ID` BIGINT(20) DEFAULT NULL COMMENT '记录id, 构件下载时为空',
  `DOWNLOAD_SPEED` BIGINT(20) NOT NULL COMMENT '下载速度',
  `DOWNLOAD_TYPE` varchar(32) NOT NULL COMMENT '下载类型',
  `CREATE_TIME` datetime NOT NULL COMMENT '创建时间',
  `UPDATE_TIME` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`ID`),
  KEY `IDX_CREATE_TIME` (`CREATE_TIME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='下载速度记录';
