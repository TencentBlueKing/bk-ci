...
CREATE TABLE IF NOT EXISTS `T_PIPELINE_ARTIFACT_INFO` (
  ...
  `ARTIFACT_TYPE`     varchar(32)  NOT NULL COMMENT '产出物类型',
  `ARTIFACT_NAME`     varchar(256) NOT NULL COMMENT '产出物名称',
  `ARTIFACT_VERSION`  varchar(128) NOT NULL DEFAULT '' COMMENT '产出物版本',
  `ARTIFACT_URI`      varchar(1024) DEFAULT NULL COMMENT '产出物唯一资源标识',
  `ARTIFACT_REPO_URL` varchar(512)  DEFAULT NULL COMMENT '产出物仓库地址',
  `ARTIFACT_DIGEST`   varchar(128)  DEFAULT NULL COMMENT '产出物摘要',
  `ARTIFACT_SIZE`     bigint(20)    DEFAULT NULL COMMENT '产出物大小',
  `CODE_REPO_URL`     varchar(512)  DEFAULT NULL COMMENT '代码库地址',
  `COMMIT_ID`         varchar(64)  NOT NULL DEFAULT '' COMMENT '代码提交ID',
  `EXTRA_INFO`        text          DEFAULT NULL COMMENT '扩展元数据JSON',
  ...
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UK_TPAI_BUILD_TASK_ARTIFACT` (`BUILD_ID`, `TASK_ID`, ARTIFACT_TYPE, `ARTIFACT_NAME`, `ARTIFACT_VERSION`),
  KEY `INX_TPAI_PROJECT_TYPE_NAME_VERSION` (`PROJECT_ID`, `ARTIFACT_TYPE`, `ARTIFACT_NAME`, `ARTIFACT_VERSION`),
  KEY `INX_TPAI_PROJECT_TYPE_NAME_VERSION_CT` (`PROJECT_ID`, `ARTIFACT_TYPE`, `ARTIFACT_NAME`, `ARTIFACT_VERSION`, `CREATE_TIME`),
