...
CREATE TABLE IF NOT EXISTS `T_WORKSPACE` (
    ...
    `OS` varchar(16) NOT NULL DEFAULT '' COMMENT '云桌面系统：LINUX/MACOS/WINDOWS',
...
CREATE TABLE IF NOT EXISTS `T_WORKSPACE_HISTORY` (
    ...
    `CREATED_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    KEY `IDX_WS_NAME_TIME` (`WORKSPACE_NAME`, `CREATED_TIME` DESC)
...
CREATE TABLE IF NOT EXISTS `T_WORKSPACE_SHARED` (
    ...
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_WORKSPACE_SHARED_USER_TYPE` (`WORKSPACE_NAME`,`SHARED_USER`,`ASSIGN_TYPE`),
    KEY `IDX_SHARED_USER` (`SHARED_USER`),
...
CREATE TABLE IF NOT EXISTS `T_WINDOWS_RESOURCE_ZONE` (
    ...
    `PROVIDER` varchar(32) NOT NULL DEFAULT 'IEG_BKCI' COMMENT '资源提供商，如 IEG_BKCI/CSIG/TEG_DEVCLOUD',
    PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_WINDOWS_RESOURCE_TYPE` (
	...
	`MACHINE_FLAG` varchar(64) DEFAULT '' COMMENT '机型标识：高配开发机、高配美术机',
    `CREATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `UPDATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`ID`),
    KEY `idx_size` (`SIZE`)
...
CREATE TABLE IF NOT EXISTS `T_PROJECT_IMAGES` (
    ...
    UNIQUE KEY `IDX_PROJECT_ID_IMAGE_ID` (`PROJECT_ID`,`IMAGE_ID`)
...
CREATE TABLE IF NOT EXISTS `T_WINDOWS_SPEC_RESOURCE` (
	`PROJECT_ID` varchar(64)  COLLATE utf8_general_ci DEFAULT '' NOT NULL COMMENT '项目ID',
	`SIZE` varchar(10)  COLLATE utf8_general_ci DEFAULT '' NOT NULL COMMENT '资源类型',
...
CREATE TABLE IF NOT EXISTS `T_WORKSPACE_NOTIFY_HISTORY`
(
...
CREATE TABLE IF NOT EXISTS `T_WORKSPACE_RECORD_USER_APPROVAL` (
	`WORKSPACE_NAME` varchar(128)  COLLATE utf8_general_ci NOT NULL COMMENT '工作空间名称',
	`USER` varchar(64) COLLATE utf8mb4_general_ci NOT NULL COMMENT '申请的用户',
	`PROJECT_ID` varchar(64)  COLLATE utf8_general_ci NOT NULL COMMENT '项目ID',
	`CREATE_TIME` timestamp DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '创建时间',
	`UPDATE_TIME` timestamp DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '修改时间',
...
-- ----------------------------
-- Table structure for T_WORKSPACE_GROUP 云桌面分组表
-- ----------------------------
CREATE TABLE IF NOT EXISTS `T_WORKSPACE_GROUP` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='云桌面分组表';

-- ----------------------------
-- Table structure for T_WORKSPACE_GROUP_RELATION 云桌面分组与工作空间关联表
-- ----------------------------
CREATE TABLE IF NOT EXISTS `T_WORKSPACE_GROUP_RELATION` (
    `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `PROJECT_ID` varchar(64) NOT NULL COMMENT '项目ID',
    ...
    `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UK_PROJECT_GROUP_WORKSPACE` (`PROJECT_ID`,`GROUP_ID`,`WORKSPACE_NAME`),
    KEY `IDX_GROUP_ID` (`GROUP_ID`),
    KEY `IDX_WORKSPACE_NAME` (`WORKSPACE_NAME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='云桌面分组与工作空间关联表';
...
CREATE TABLE IF NOT EXISTS `T_REMOTEDEV_PROJECT_STRATEGY` (
    `ID` bigint(11) auto_increment NOT NULL COMMENT 'ID',
	`PROJECT_ID` varchar(64)  COLLATE utf8_general_ci NOT NULL COMMENT '项目ID',
	`ZONE_TYPE` varchar(32)  COLLATE utf8_general_ci DEFAULT 'DEFAULT' NOT NULL COMMENT '区分类型，以满足特殊区域',
	`STRATEGY_TYPE` varchar(32) NOT NULL COMMENT '策略类型',
	`STRATEGY_CONTENT` json NOT NULL COMMENT '具体策略',
    PRIMARY KEY (`ID`),
    UNIQUE INDEX `strategy`(`PROJECT_ID`, `ZONE_TYPE`, `STRATEGY_TYPE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT = '项目策略表';
