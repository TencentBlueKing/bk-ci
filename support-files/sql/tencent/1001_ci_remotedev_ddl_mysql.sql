...
CREATE TABLE IF NOT EXISTS `T_PROJECT_IMAGES` (
    `ID` bigint(11) NOT NULL AUTO_INCREMENT COMMENT 'ID',
    `PROJECT_ID` varchar(10) NOT NULL DEFAULT '' COMMENT '蓝盾项目ID',
...
CREATE TABLE IF NOT EXISTS `T_PROJECT_SOFTWARES` (
    ...
    `INSTALL_METHOD` varchar(10) NOT NULL DEFAULT '' COMMENT '安装方式',
    `CREATOR` varchar(32) NOT NULL DEFAULT '' COMMENT '创建人',
    `CREATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_USER_INSTALLED_SOFTWARES` (
    ...
    `SOFTWARE_NAME` varchar(128) NOT NULL DEFAULT '' COMMENT '应用名称',
    ...
    PRIMARY KEY (`ID`),
    UNIQUE `ukey`(`PROJECT_ID`,`CREATOR`,`SOFTWARE_NAME`,`WORKSPACE_NAME`)
