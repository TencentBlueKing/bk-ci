...
CREATE TABLE IF NOT EXISTS `T_WORKSPACE` (
    ...
    `STATUS` int(11) NOT NULL DEFAULT 0 COMMENT '工作空间状态,0-PREPARING,1-RUNNING,2-STOPPED,3-SLEEP,4-DELETED,5-EXCEPTION,6-STARTING,7-SLEEPING,8-DELETING,9-DELIVERING,10-DISTRIBUTING',
...
CREATE TABLE IF NOT EXISTS `T_PROJECT_IMAGES` (
    ...
    `SIZE` varchar(64) NOT NULL DEFAULT '' COMMENT '镜像大小，单位G',
    `ZONE` varchar(64) NOT NULL DEFAULT '' COMMENT '区域：深圳等',
...
CREATE TABLE IF NOT EXISTS `T_PROJECT_SOFTWARES` (
    ...
    `CLASSIFICATION` varchar(128) NOT NULL DEFAULT '' COMMENT '分类',
    `INSTALL_METHOD` varchar(128) NOT NULL DEFAULT '' COMMENT '安装方式',
    `CREATOR` varchar(32) NOT NULL DEFAULT '' COMMENT '创建人',
    `CREATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`ID`),
