...
(
    ...
    INDEX `IDX_SN` (`SN`),
    PRIMARY KEY (`ID`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT '项目审批回调表';

CREATE TABLE `T_SIGNATURE_PLATFORM_DETAILS` (
    `PLATFORM` varchar(32) NOT NULL COMMENT '业务方平台标识',
    `PLATFORM_SECRET` varchar(128) NOT NULL COMMENT '业务方平台秘钥',
    `PLATFORM_NAME` varchar(255) NOT NULL COMMENT '业务方平台名称',
    `URL` mediumtext COMMENT '请求地址',
    `INFORMATION_CN` mediumtext COMMENT '文案信息-中文',
    `INFORMATION_EN` mediumtext COMMENT '文案信息-英文',
    `PROJECT_IDS` mediumtext COMMENT '项目信息，多个项目以逗号隔开',
    `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`PLATFORM`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='保密签业务平台信息表';
