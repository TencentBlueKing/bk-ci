...
CREATE TABLE IF NOT EXISTS `T_NODE` (
  ...
  KEY `IDX_STAT_TYPE` (`NODE_STATUS`,`NODE_TYPE`),
  KEY `IDX_HOST_PROJ` (`HOST_ID`,`PROJECT_ID`),
  KEY `IDX_TYPE_HOST` (`NODE_TYPE`,`HOST_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `T_BKBIZ_PROJECT`  (
  ...
  `ENABLE_MONITOR_DASHBOARD` bit(1) NULL COMMENT '是否开启蓝鲸监控仪表盘',
  PRIMARY KEY (`ID`),
  INDEX `PROJECT_ID_IDX`(`PROJECT_ID`) USING BTREE
)ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT='蓝鲸BIZ和蓝盾项目映射表';
