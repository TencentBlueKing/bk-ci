...
CREATE TABLE IF NOT EXISTS `T_AI_AGENT_STATE` (
  ...
  `CREATED_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `UPDATED_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  PRIMARY KEY (`SESSION_ID`, `STATE_KEY`, `ITEM_INDEX`, `CREATED_TIME`)
...
CREATE TABLE IF NOT EXISTS `T_AI_RUN_EVENT` (
  ...
  `CREATED_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  PRIMARY KEY (`ID`, `CREATED_TIME`),
  UNIQUE INDEX `UNI_THREAD_RUN_INDEX` (`THREAD_ID`, `RUN_ID`, `EVENT_INDEX`, `CREATED_TIME`),
  INDEX `IDX_CREATED_TIME` (`CREATED_TIME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='AI运行事件缓冲表（回放用，用完即删）';

-- ----------------------------
-- Table structure for T_AI_USER_LLM_CONFIG
-- ----------------------------

CREATE TABLE IF NOT EXISTS `T_AI_USER_LLM_CONFIG` (
    `USER_ID` varchar(64) NOT NULL COMMENT '归属用户',
    `BASE_URL` varchar(512) NOT NULL COMMENT '模型服务地址',
    `MODEL_NAME` varchar(128) NOT NULL COMMENT '模型名称',
    `API_KEY` text DEFAULT NULL COMMENT '加密后的 OpenAI 兼容模式 API Key',
    `BK_APP_CODE` varchar(128) NOT NULL DEFAULT '' COMMENT '蓝鲸网关模式 bk_app_code',
    `BK_APP_SECRET` text DEFAULT NULL COMMENT '加密后的蓝鲸网关模式 bk_app_secret',
    `ENABLED` bit(1) NOT NULL DEFAULT b'1' COMMENT '是否启用',
    `CONNECT_TIMEOUT_SECONDS` bigint(20) NOT NULL DEFAULT 10 COMMENT 'TCP 连接超时（秒）',
    `READ_TIMEOUT_SECONDS` bigint(20) NOT NULL DEFAULT 90 COMMENT 'HTTP 读超时（秒）',
    `WRITE_TIMEOUT_SECONDS` bigint(20) NOT NULL DEFAULT 30 COMMENT '请求写入超时（秒）',
    `EXECUTION_TIMEOUT_SECONDS` bigint(20) NOT NULL DEFAULT 60 COMMENT '单次模型调用超时（秒）',
    `MAX_ATTEMPTS` int(11) NOT NULL DEFAULT 5 COMMENT '最大尝试次数（含首次调用）',
    `INITIAL_BACKOFF_SECONDS` bigint(20) NOT NULL DEFAULT 1 COMMENT '首次重试退避间隔（秒）',
    `MAX_BACKOFF_SECONDS` bigint(20) NOT NULL DEFAULT 8 COMMENT '最大重试退避间隔（秒）',
    `BACKOFF_MULTIPLIER` double NOT NULL DEFAULT 2 COMMENT '退避倍数',
    `CREATED_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    `UPDATED_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (`USER_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户自定义大模型配置表';

-- ----------------------------
-- Table structure for T_AI_KB_SOURCE / T_AI_KB_ENTRY
-- ----------------------------
