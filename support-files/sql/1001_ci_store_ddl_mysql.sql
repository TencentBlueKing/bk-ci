...
CREATE TABLE IF NOT EXISTS `T_ATOM` (
  ...
  `ATOM_STATUS` tinyint(4) NOT NULL COMMENT '插件状态',
...
CREATE TABLE IF NOT EXISTS `T_ATOM_VERSION_LOG` (
  ...
  `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  `PACKAGE_SIZE` text DEFAULT NULL COMMENT '版本包大小',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_CLASSIFY` (
  ...
  `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  `TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT '类型',
  `SERVICE_SCOPE` varchar(64) DEFAULT '' COMMENT '服务范围，PIPELINE：流水线 CREATIVE_STREAM：创作流',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UNI_INX_TC_NAME_TYPE_SCOPE` (`TYPE`,`SERVICE_SCOPE`,`CLASSIFY_NAME`),
  UNIQUE KEY `UNI_INX_TC_CODE_TYPE_SCOPE` (`TYPE`,`SERVICE_SCOPE`,`CLASSIFY_CODE`)
...
CREATE TABLE IF NOT EXISTS `T_LABEL` (
  ...
  `SERVICE_SCOPE` varchar(64) DEFAULT '' COMMENT '服务范围，PIPELINE：流水线 CREATIVE_STREAM：创作流',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UNI_INX_TL_TYPE_NAME_SCOPE` (`TYPE`,`SERVICE_SCOPE`,`LABEL_NAME`),
  UNIQUE KEY `UNI_INX_TL_TYPE_CODE_SCOPE` (`TYPE`,`SERVICE_SCOPE`,`LABEL_CODE`)
...
CREATE TABLE IF NOT EXISTS `T_STORE_MEMBER` (
  ...
  `STORE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT 'store组件类型',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UNI_INX_TSM_TYPE_CODE_NAME`(`STORE_TYPE`, `STORE_CODE`,`USERNAME`),
...
CREATE TABLE IF NOT EXISTS `T_STORE_PROJECT_REL` (
  ...
  `INSTANCE_ID` varchar(256) COMMENT '实例ID',
  `INSTANCE_NAME` varchar(256) COMMENT '实例名称',
  `CREATOR` varchar(50) NOT NULL DEFAULT 'system' COMMENT '创建者',
  `MODIFIER` varchar(50) NOT NULL DEFAULT 'system' COMMENT '修改者',
  `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  `STORE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT 'store组件类型',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UNI_INX_TSPR_STORE_PROJECT_TYPE_INSTANCE_CREATOR` (`STORE_CODE`,`STORE_TYPE`,`PROJECT_CODE`,`TYPE`,`INSTANCE_ID`,`CREATOR`),
  KEY `INX_TSPR_TYPE_PROJECT_TYPE_INSTANCE` (`STORE_TYPE`,`PROJECT_CODE`,`TYPE`,`INSTANCE_ID`),
  KEY `INX_TSPR_TYPE_CODE_TYPE` (`STORE_TYPE`,`STORE_CODE`,`TYPE`)
...
CREATE TABLE IF NOT EXISTS `T_STORE_STATISTICS_TOTAL` (
  ...
  `RECENT_ACTIVE_DURATION` decimal(15,2) COMMENT '总活跃时长，单位：小时',
  `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS `T_STORE_STATISTICS_DAILY` (
  ...
  `DAILY_ACTIVE_DURATION` decimal(15,2) COMMENT '每日活跃时长，单位：小时',
...
CREATE TABLE IF NOT EXISTS `T_STORE_BASE` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键ID',
    `STORE_CODE` varchar(64) NOT NULL COMMENT '组件唯一标识',
    `STORE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT 'store组件类型',
    `NAME` varchar(64) NOT NULL COMMENT '组件名称',
    `VERSION` varchar(256) NOT NULL COMMENT '版本号',
    `STATUS` varchar(64) NOT NULL COMMENT '状态',
    `STATUS_MSG` varchar(1024) DEFAULT NULL COMMENT '状态对应的描述，如上架失败原因',
    `DOCS_LINK` varchar(256) DEFAULT NULL COMMENT '说明文档链接',
    `LOGO_URL` varchar(256) DEFAULT NULL COMMENT 'logo地址',
    `SUMMARY` varchar(256) DEFAULT NULL COMMENT '组件简介',
    `DESCRIPTION` mediumtext COMMENT '组件描述',
    `LATEST_FLAG` bit(1) NOT NULL COMMENT '是否为最新版本， TRUE：最新 FALSE：非最新',
    `PUBLISHER` varchar(1024) NOT NULL DEFAULT 'system' COMMENT '发布者,对应T_STORE_PUBLISHER_INFO表的PUBLISHER_NAME字段',
    `PUB_TIME` datetime(3) DEFAULT NULL COMMENT '发布时间',
    `CLASSIFY_ID` varchar(32) NOT NULL COMMENT '所属分类ID',
    `BUS_NUM` bigint(20) DEFAULT NULL COMMENT '业务序号',
    `CREATOR` varchar(50) NOT NULL DEFAULT 'system' COMMENT '创建人',
    `MODIFIER` varchar(50) NOT NULL DEFAULT 'system' COMMENT '最近修改人',
    `UPDATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '修改时间',
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_INX_TSB_TYPE_CODE_VERSION` (`STORE_TYPE`,`STORE_CODE`,`VERSION`),
    KEY `INX_TSB_TYPE_NAME_LATEST_FLAG` (`STORE_TYPE`,`NAME`,`LATEST_FLAG`),
    KEY `INX_TSB_TYPE_STATUS` (`STORE_TYPE`,`STATUS`),
    KEY `INX_TSB_TYPE_CLASSIFY_ID` (`STORE_TYPE`,`CLASSIFY_ID`),
    KEY `INX_TSB_TYPE_CODE_NUM` (`STORE_TYPE`,`STORE_CODE`,`BUS_NUM`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件基本信息表';

CREATE TABLE IF NOT EXISTS `T_STORE_BASE_EXT` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键ID',
    `STORE_ID` varchar(32) NOT NULL DEFAULT '' COMMENT '组件基本信息表ID',
    `STORE_CODE` varchar(64) NOT NULL COMMENT '组件唯一标识',
    `STORE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT 'store组件类型',
    `FIELD_NAME` varchar(64) NOT NULL DEFAULT '' COMMENT '字段名称',
    `FIELD_VALUE` mediumtext NOT NULL  COMMENT '字段值',
    `CREATOR` varchar(50) NOT NULL DEFAULT 'system' COMMENT '创建人',
    `MODIFIER` varchar(50) NOT NULL DEFAULT 'system' COMMENT '最近修改人',
    `UPDATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '修改时间',
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_INX_TSBE_ID_NAME` (`STORE_ID`,`FIELD_NAME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件基本信息扩展表';

CREATE TABLE IF NOT EXISTS `T_STORE_BASE_FEATURE` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键ID',
    `STORE_CODE` varchar(64) NOT NULL COMMENT '组件唯一标识',
    `STORE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT 'store组件类型',
    `PUBLIC_FLAG` bit(1) DEFAULT b'0' COMMENT '是否为公共组件， TRUE：是 FALSE：不是',
    `RECOMMEND_FLAG` bit(1) DEFAULT b'1' COMMENT '是否推荐， TRUE：是 FALSE：不是',
    `CERTIFICATION_FLAG` bit(1) DEFAULT b'0' COMMENT '是否官方认证， TRUE：是 FALSE：不是',
    `TYPE` varchar(32) DEFAULT NULL COMMENT '类型',
    `RD_TYPE` varchar(32) DEFAULT NULL COMMENT '研发类型',
    `WEIGHT` int(11) DEFAULT NULL COMMENT '权重（数值越大代表权重越高）',
    `SHOW_FLAG` bit(1) DEFAULT b'1' COMMENT '是否在首页展示， TRUE：是 FALSE：不是',
    `CREATOR` varchar(50) NOT NULL DEFAULT 'system' COMMENT '创建人',
    `MODIFIER` varchar(50) NOT NULL DEFAULT 'system' COMMENT '最近修改人',
    `UPDATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '修改时间',
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_INX_TSBF_TYPE_CODE` (`STORE_TYPE`,`STORE_CODE`),
    KEY `INX_TSBF_TYPE_CODE_PUBLIC_FLAG` (`STORE_TYPE`,`STORE_CODE`,`PUBLIC_FLAG`),
    KEY `INX_TSBF_TYPE_CODE_RECOMMEND_FLAG` (`STORE_TYPE`,`STORE_CODE`,`RECOMMEND_FLAG`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件特性信息表';

CREATE TABLE IF NOT EXISTS `T_STORE_BASE_FEATURE_EXT` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键ID',
    `FEATURE_ID` varchar(32) NOT NULL DEFAULT '' COMMENT '组件特性信息表ID',
    `STORE_CODE` varchar(64) NOT NULL COMMENT '组件唯一标识',
    `STORE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT 'store组件类型',
    `FIELD_NAME` varchar(64) NOT NULL DEFAULT '' COMMENT '字段名称',
    `FIELD_VALUE` text NOT NULL  COMMENT '字段值',
    ...
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_INX_TSBFE_TYPE_CODE_NAME` (`STORE_TYPE`,`STORE_CODE`,`FIELD_NAME`),
    KEY `INX_TSBFE_FEATURE_ID` (`FEATURE_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件特性信息扩展表';

CREATE TABLE IF NOT EXISTS `T_STORE_BASE_ENV` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键',
    `STORE_ID` varchar(32) NOT NULL DEFAULT '' COMMENT '组件基本信息表ID',
    `LANGUAGE` varchar(64) DEFAULT NULL COMMENT '开发语言',
    `MIN_VERSION` varchar(20) DEFAULT NULL COMMENT '支持开发语言的最低版本',
    `PKG_NAME` varchar(256) DEFAULT NULL COMMENT '包名称',
    `PKG_PATH` varchar(1024) DEFAULT NULL COMMENT '包路径',
    `TARGET` varchar(256) DEFAULT NULL COMMENT '执行入口',
    `SHA_CONTENT` varchar(1024) DEFAULT NULL COMMENT 'SHA1签名串',
    `SHA256_CONTENT` varchar(1024) DEFAULT NULL COMMENT 'SHA256签名串',
    `PRE_CMD` text COMMENT '执行前置命令',
    `OS_NAME` varchar(128) DEFAULT NULL COMMENT '支持的操作系统名称',
    `OS_ARCH` varchar(128) DEFAULT NULL COMMENT '支持的操作系统架构',
    `RUNTIME_VERSION` varchar(128) DEFAULT NULL COMMENT '运行时版本',
    `DEFAULT_FLAG` bit(1) NOT NULL DEFAULT b'1' COMMENT '是否为默认环境信息',
    ...
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_INX_TSBE_ID_OS_NAME_ARCH` (`STORE_ID`,`OS_NAME`,`OS_ARCH`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件执行环境信息表';

CREATE TABLE IF NOT EXISTS `T_STORE_BASE_ENV_EXT` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键ID',
    `ENV_ID` varchar(32) NOT NULL DEFAULT '' COMMENT '组件执行环境信息表ID',
    `STORE_ID` varchar(32) NOT NULL DEFAULT '' COMMENT '组件基本信息表ID',
    `FIELD_NAME` varchar(64) NOT NULL DEFAULT '' COMMENT '字段名称',
    `FIELD_VALUE` mediumtext NOT NULL  COMMENT '字段值',
    `CREATOR` varchar(50) NOT NULL DEFAULT 'system' COMMENT '创建人',
    ...
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_INX_TSBEE_ENV_ID_NAME` (`ENV_ID`,`FIELD_NAME`),
    KEY `INX_TSBEE_STORE_ID` (`STORE_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件执行环境信息扩展表';

CREATE TABLE IF NOT EXISTS `T_STORE_CATEGORY_REL` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键',
    `CATEGORY_ID` varchar(32) NOT NULL COMMENT '范畴ID',
    `STORE_ID` varchar(32) NOT NULL COMMENT '组件ID',
    `CREATOR` varchar(50) NOT NULL DEFAULT 'system' COMMENT '创建人',
    `MODIFIER` varchar(50) NOT NULL DEFAULT 'system' COMMENT '最近修改人',
    `UPDATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '修改时间',
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    KEY `INX_TSCR_CATEGORY_ID` (`CATEGORY_ID`),
    KEY `INX_TSCR_STORE_ID` (`STORE_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件与范畴关联关系表';

CREATE TABLE IF NOT EXISTS `T_STORE_LABEL_REL` (
    `ID` varchar(32) NOT NULL COMMENT '主键',
    `LABEL_ID` varchar(32) NOT NULL COMMENT '标签ID',
    `STORE_ID` varchar(32) NOT NULL COMMENT '组件ID',
    `CREATOR` varchar(50) NOT NULL DEFAULT 'system' COMMENT '创建人',
    ...
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    KEY `INX_TSLR_LABEL_ID` (`LABEL_ID`),
    KEY `INX_TSLR_STORE_ID` (`STORE_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件与标签关联关系表';

CREATE TABLE IF NOT EXISTS `T_STORE_VERSION_LOG` (
    `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键',
    `STORE_ID` varchar(32) NOT NULL COMMENT '组件ID',
    `RELEASE_TYPE` tinyint(4) NOT NULL COMMENT '发布类型，0：新上架 1：非兼容性升级 2：兼容性功能更新 3：兼容性问题修正',
    `CONTENT` text NOT NULL COMMENT '版本日志内容',
    ...
    `PACKAGE_SIZE` text DEFAULT NULL COMMENT '版本包大小',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UNI_INX_TSVL_STORE_ID` (`STORE_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='研发商店组件版本日志表';

CREATE TABLE IF NOT EXISTS `T_TEMPLATE_VERSION_RELEASED_REL` (
    ...
    `NUMBER`  int(11) NOT NULL COMMENT '版本排序号',
    `VERSION_NAME` varchar(64) NOT NULL COMMENT '模板版本号',
    `PUBLISHED` bit(1) NOT NULL COMMENT '是否上架研发商店',
    `CREATOR` varchar(64) NOT NULL COMMENT '创建者',
    `UPDATER` varchar(64) DEFAULT NULL COMMENT '修改人',
    `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`TEMPLATE_CODE`,`VERSION`) USING BTREE,
    KEY `inx_template_code` (`TEMPLATE_CODE`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='模板上架研发商店版本记录表';

CREATE TABLE IF NOT EXISTS `T_TEMPLATE_VERSION_INSTALL_HISTORY` (
    ...
    `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`ID`) USING BTREE,
    UNIQUE KEY `uni_index` (`SRC_MARKET_TEMPLATE_CODE`,`TEMPLATE_CODE`,`VERSION`) USING BTREE,
    KEY `inx_template_code` (`TEMPLATE_CODE`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='模板版本安装历史表';
SET FOREIGN_KEY_CHECKS = 1;

CREATE TABLE IF NOT EXISTS `T_ATOM_WHITELIST` (
  `ID` varchar(32) NOT NULL DEFAULT '' COMMENT '主键ID',
  `WHITELIST_TYPE` varchar(64) NOT NULL COMMENT '白名单类型',
  `ATOM_CODES` text COMMENT '插件代码列表（JSON数组）',
  `DESCRIPTION` text COMMENT '描述',
  ...
  `UPDATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `uk_whitelist_type` (`WHITELIST_TYPE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='插件功能白名单表';
