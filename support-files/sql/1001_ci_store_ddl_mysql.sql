...
CREATE TABLE IF NOT EXISTS `T_STORE_DOCKING_PLATFORM` (
  ...
  `ERROR_CODE_PREFIX` int(3) DEFAULT NULL COMMENT '平台所属错误码前缀',
  PRIMARY KEY (`ID`),
  ...
  UNIQUE KEY `UNI_INX_TSDP_PLATFORM_ERROR` (`ERROR_CODE_PREFIX`)
...
CREATE TABLE IF NOT EXISTS  `T_STORE_DOCKING_PLATFORM_ERROR_CODE` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UNI_TSDPEC_PLATFORM_CODE` (`PLATFORM_CODE`),
