...
CREATE TABLE IF NOT EXISTS `T_STORE_STATISTICS_TOTAL` (
  ...
  `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  `HOT_FLAG` bit(1) DEFAULT b'1' COMMENT '热度是否到达80分位',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_STORE_DOCKING_PLATFORM` (
  ...
  `LABELS` varchar(1024) NOT NULL COMMENT '标签列表',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_STORE_ERROR_CODE_INFO` (
    `ID` varchar(32) NOT NULL COMMENT '主键',
    `STORE_CODE` varchar(64) NOT NULL COMMENT '商店代码',
    ...
    `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    `ERROR_CODE_TYPE` tinyint(4) NOT NULL DEFAULT '0' COMMENT '错误码所属类型 0:组件，1：第三方平台，2：通用',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `T_STORE_ERROR_CODE_INFO_UN` (`STORE_CODE`,`ERROR_CODE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='store错误码信息';
...
CREATE TABLE IF NOT EXISTS  `T_STORE_INDEX_ELEMENT_DETAIL` (
    ...
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `T_STORE_INDEX_ELEMENT_DETAIL_UN` (`STORE_CODE`,`STORE_TYPE`,`INDEX_ID`,`INDEX_CODE`),
    KEY `INX_TSIR_TYPE_CODE` (`STORE_TYPE`,`STORE_CODE`,`INDEX_CODE`),
...
CREATE TABLE IF NOT EXISTS  `T_STORE_INDEX_RESULT` (
    ...
    `CREATE_TIME` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `T_STORE_INDEX_RESULT_UN` (`STORE_CODE`,`STORE_TYPE`,`INDEX_ID`,`INDEX_CODE`),
    KEY `INX_TSIR_TYPE_CODE` (`STORE_TYPE`,`STORE_CODE`,`INDEX_CODE`),
    KEY `INX_TSIR_INDEX_ID` (`INDEX_ID`),
