...
CREATE TABLE IF NOT EXISTS T_AUTH_RESOURCE_GROUP (
    ...
    `APPLY_DISABLE` bit(1) DEFAULT NULL COMMENT '是否禁止申请，当true为禁止',
	PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `T_AUTH_PROJECT_RESET_RECORD` (
    `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `TASK_ID` varchar(64) NOT NULL COMMENT '任务ID',
    `PROJECT_CODE` varchar(64) NOT NULL COMMENT '项目代码',
    `RESET_TYPE` varchar(32) NOT NULL COMMENT '重置类型：FULL-完整重置, RESOURCE-资源重置, PERMISSION-权限重置',
    `MIGRATE_RESOURCE` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否迁移资源',
    `FILTER_RESOURCE_TYPES` text DEFAULT NULL COMMENT '过滤的资源类型列表(JSON格式)',
    `FILTER_ACTIONS` text DEFAULT NULL COMMENT '过滤的操作列表(JSON格式)',
    ...
    INDEX `IDX_STATUS` (`STATUS`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目权限重置记录表';
