...
CREATE TABLE `T_AUTH_SYNC_DATA_TASK` (
...
CREATE TABLE T_AUTH_MEMBER_PROJECT_PERMISSION (
    ...
    `CREATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE  KEY `UNI_KEY` (`MEMBER_ID`, `PROJECT_CODE`, `ACTION`, `IAM_GROUP_ID`),
    INDEX `PROJECT_CODE_AND_GROUP_ID_INEDX` (`PROJECT_CODE`,`IAM_GROUP_ID`)
) COMMENT='用户项目级权限表';
