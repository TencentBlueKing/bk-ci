...
-- ----------------------------
-- Table structure for T_REPOSITORY_SCM_CONFIG_DEPT_REL
-- ----------------------------
CREATE TABLE `T_REPOSITORY_SCM_CONFIG_DEPT_REL` (
    ...
    `CREATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `IDX_RSCD_UNIQUE` (`SCM_CODE`,`DEPT_ID`),
    KEY `IDX_RSCD_DEPT_ID` (`DEPT_ID`),
    KEY `IDX_RSCD_SCM_CODE` (`SCM_CODE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='代码源与与机构关联关系表'
