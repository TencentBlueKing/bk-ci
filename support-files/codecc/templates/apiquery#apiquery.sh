...
SERVICE_NAME=apiquery
HTTP_PORT=__APIQUERY_PORT__
...
function startup() {
    ...
    JAVA_OPTS="$JOB_JAVA_OPTS -Dserver.address=__LAN_IP__"
    JAVA_OPTS="$JAVA_OPTS -server -Dfile.encoding=UTF-8 -Djava.security.egd=file:/dev/./urandom"
    JAVA_OPTS="$JAVA_OPTS -Dspring.config.location=file:${CONF_HOME}/application-${SERVICE_NAME}.yml"
