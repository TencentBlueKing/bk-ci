...
# 服务相关修改
SERVICE_NAME=bkcodeccservice
# HTTP端口的是占位符号 需要根据application-bkcodeccservice.yml中设置的端口来替换
HTTP_PORT=__BK_CODECC_BKCODECCSERVICE_API_PORT__
...
#    JAVA_OPTS="$JAVA_OPTS -server -Dcertificate.file=${CERT_PATH}/bkcodecc_platform.cert -Dfile.encoding=UTF-8 -Djava.security.egd=file:/dev/./urandom"
    ...
    JAVA_OPTS="$JAVA_OPTS -Ddevops_gateway=__BK_HTTP_SCHEMA__://__BK_CODECC_FQDN__"
