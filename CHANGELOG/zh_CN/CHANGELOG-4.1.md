...
### 变更概述
当前版本主要变更特性如下:
- 模版 as Code，支持通过 YAML 文件编排模版
- 模版版本管理重构，支持差异对比、回滚、删除历史版本
- 模版增加查看页面
- 模版编排对齐流水线
- 模版实例化时，支持高亮变更参数、支持批量修改入参；支持在实例启/停触发器
- 模版实例化时，支持在实例里修改定时触发器规则
- 执行流水线时支持以指定组合的入参启动
- 变量支持设置是否敏感
- 流水线支持设置取消策略
- MacOS构建机支持指定资源规格配置
- dynamic-parameter 和 dynamic-parameter-simple 组件中的 select 和 enum-input 支持输入变量
- 流水线插件 SDK：企业微信机器人消息通知支持发送图片
- 研发商店镜像支持升级历史小版本
- 增加 ci 上下文变量 ci.debug 标识当前构建是调试还是正式
- 支持 web 端发起 windows 版本体验
...
- [新增] 蓝鲸网关配置更新 [链接](http://github.com/TencentBlueKing/bk-ci/issues/12169)
...
- [修复] bugfix: 排队调度没有区分系统 [链接](http://github.com/TencentBlueKing/bk-ci/issues/12368)
- [修复] bug: 修复流水线导出接口 [链接](http://github.com/TencentBlueKing/bk-ci/issues/12243)
- [修复] bug: 项目迁移数据操作锁定优化 [链接](http://github.com/TencentBlueKing/bk-ci/issues/12261)
