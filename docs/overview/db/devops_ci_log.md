...
| 表名                  | 说明       |
| :---: | :---: |
| T_LOG_BUILD_PROJECT | 构建与项目/流水线映射，供日志流量聚合与查询鉴权 |
| T_LOG_INDICES_V2 | 构建日志已关联ES索引表 |
| T_LOG_STATUS | 构建日志打印状态表 |
| T_LOG_SUBTAGS | 构建日志子标签表 |

**表名：** <a>T_LOG_BUILD_PROJECT</a>

**说明：** 构建与项目/流水线映射，供日志流量聚合与查询鉴权
...
|  2   | PROJECT_ID |   varchar   | 64 |   0    |    Y     |  N   |       | 项目ID，空表示尚未上报  |
|  3   | PIPELINE_ID |   varchar   | 64 |   0    |    Y     |  N   |       | 流水线ID，空表示尚未上报  |
|  4   | CREATED_TIME |   datetime   | 19 |   0    |    N     |  N   |   CURRENT_TIMESTAMP    | 创建时间  |
|  5   | UPDATED_TIME |   datetime   | 19 |   0    |    N     |  N   |   CURRENT_TIMESTAMP    | 更新时间  |
...
|  6   | USER_JOB_ID |   varchar   | 128 |   0    |    Y     |  N   |       | 真正的jobId，已经存在的JOB_ID字段其实是containerhashid  |
|  7   | STEP_ID |   varchar   | 64 |   0    |    Y     |  N   |       | 用户填写的插件id  |
