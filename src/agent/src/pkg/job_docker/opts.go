...
// 从docker cli https://github.com/docker/cli/blob/424401233fd9596f2073a487c6b4329570a18a2b/cli/command/container/opts.go
// 中复制修改的私有类型和方法，用来解析docker flags
...
func parse(flags *pflag.FlagSet, copts *containerOptions, serverOS string) (*ContainerConfig, error) {
	...
	if len(mounts) > 0 && copts.volumeDriver != "" {
		logs.Warn("`--volume-driver` is ignored for volumes specified via `--mount`. Use `--mount type=volume,volume-driver=...` instead.")
	}
	var binds []string
