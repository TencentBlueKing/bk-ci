...
| 文档 | 说明 |
|------|------|
| [安装部署指南](installation.md) | 前置条件、各平台安装步骤、支持的部署场景 |
| [CLI 命令参考](agent-cli.md) | 所有 CLI 子命令的用法和示例 |

### 架构与配置

| 文档 | 说明 |
|------|------|
| [架构与模块说明](architecture.md) | Agent 整体架构、进程模型、模块职责、主循环流程 |
| [配置参考](configuration.md) | `.agent.properties` 参数详解、辅助文件、环境变量开关 |

### 平台指南

| 文档 | 说明 |
|------|------|
| [Linux 平台指南](platform-linux.md) | systemd 模式、Root/非 Root、容器部署、login shell |
| [macOS 平台指南](platform-macos.md) | launchd 模式、无头模式、JDK 路径差异 |
| [Windows 平台指南](platform-windows.md) | 服务模式、Session 桌面模式、环境变量自动刷新 |

### 功能专题

| 文档 | 说明 |
|------|------|
| [构建系统与工作空间](build-and-workspace.md) | 构建流程、Worker 进程、JDK 管理、磁盘占用与清理 |
| [Docker/Podman 容器构建](docker-builds.md) | 容器构建模式、Podman 兼容、挂载规则、镜像调试 |
| [升级与重装机制](upgrade-and-reinstall.md) | 自动升级流程、`reinstall` 完全重装、保留文件清单 |
| [故障排查与状态检查](troubleshooting.md) | `status` 命令解读、健康检查项、常见故障排查步骤 |
