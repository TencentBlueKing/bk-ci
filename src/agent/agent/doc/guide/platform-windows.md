...
| 安装模式 | 登录用户 | Agent获取的环境变量 |
|------|-------------------|-------------------|
| service模式（devopsAgent.exe install）（默认安装模式） | SYSTEM | 系统环境变量 |
| service模式-用户登录（devopsAgent.exe install + 服务页面登录） | 服务登录的用户 | 服务登录的用户环境变量+系统环境变量 |
| session模式（devopsAgent.exe install session） | 当前登录的用户 | 当前登录的用户+系统环境变量 |
| task模式（devopsAgent.exe install task） | 执行安装命令时登录的用户 | 执行安装命令时登录的用户+系统环境变量 |

**⚠ 注意事项：**
>
> 1. **USERNAME, USERPROFILE 环境变量都会被windows环境变量中的配置覆盖**： 如果需要获取可靠的当前用户，可以优先考虑：
>    - whoami 获取 SID
>    - 从 SHGetKnownFolderPath API 获取用户目录（更可靠）
