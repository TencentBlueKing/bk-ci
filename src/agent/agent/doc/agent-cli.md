...
| **Windows** | `service` / `session` / `task` | `service` | `service` = Windows 服务; `session` = 服务 + 桌面会话; `task` = [已废弃] 计划任务 |
...
# 配置 LogonUser 回退：没人登录时也能创建 Session
.\devopsAgent.exe install --mode session --user builduser --password P@ssw0rd

# 完整：配置 Windows 自动登录，重启后也有 Session
.\devopsAgent.exe install --mode session --user builduser --password P@ssw0rd --auto-logon
```
...
# 切换安装模式 — 直接 install 新模式即可，自动先卸载
./devopsAgent install --mode user

# 文件损坏修复（JDK 被误删等）— 一条命令搞定
./devopsAgent repair

# Windows 切换到 Session 模式
.\devopsAgent.exe install --mode session --user builduser --password xxx

# Windows 回退到普通服务模式
.\devopsAgent.exe install --mode service
```
...
| `install_schtasks_for_ui.bat` | `devopsAgent install --mode session` |
| `devopsctl.vbs` | 不再需要 |
