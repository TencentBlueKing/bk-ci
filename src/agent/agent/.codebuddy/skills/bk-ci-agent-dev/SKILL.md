...
agent.Run()
  ...
  ├─ mcp.StartIfEnabled()  // 检查环境变量，启用则在 goroutine 中启动 MCP Server（Streamable HTTP 随机端口）
  └─ 主循环 (每5秒):
...
- 端口随机分配（`:0`），避免端口冲突，不阻塞主进程
- 端口号写入 `{workDir}/.mcp_port` 文件，供 MCP 客户端读取
...
```
其中 `{PORT}` 从 `{workDir}/.mcp_port` 文件中读取。
...
- `server.go` — Streamable HTTP Server 创建、随机端口监听、端口文件写入
...
├── .cert                           # TLS证书(可选)
├── .mcp_port                       # MCP Server 端口号文件(启用MCP时自动生成)
...
8. **MCP调试**: 设置 `DEVOPS_AGENT_ENABLE_MCP=true`，agent 启动后 MCP Server 在 `127.0.0.1` 随机端口监听 Streamable HTTP，端口号写入 `{workDir}/.mcp_port` 文件。可在 CodeBuddy/Claude Desktop 中配置 `http://127.0.0.1:{PORT}/mcp` 进行交互式调试
