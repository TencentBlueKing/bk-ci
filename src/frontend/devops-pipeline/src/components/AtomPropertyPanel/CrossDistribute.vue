                ...
                try {
                    ...
                    if (res.data && res.data.length > 0) {
                        for (let i = 0; i < res.data.length; i++) {
                            if (res.data[i]['project_code'] !== this.projectId && res.data[i]['approval_status'] === 2) {
                                tmpList.push(Object.assign({}, { id: res.data[i]['project_code'], name: res.data[i]['project_name'] }))
                            }
                        }
