<template>
    <div v-bkloading="{ isLoading }" class="remote-trigger">
        ...
        <p class="curl-url">curl -X POST {{baseUrl}}/external/pipelines/{{value}}/build -H "Content-Type: application/json" -H "X-DEVOPS-PROJECT-ID: {{projectId}}" -d "{{stringifyParmas}}" </p>
    </div>
</template>
