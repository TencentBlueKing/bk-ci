    ...
    <section>
        <div>
            ...
            <a
                ...
                class="check-inline-link"
                target="_blank"
                v-if="taskId"
                @click.stop="goTurboLink"
            >{{ taskName }}</a>
        </div>
        <bk-alert
            type="error"
            class="turbo-tip"
            v-if="taskId && turboValue"
        >
            <div slot="title">
                当前使用的是旧版 distcc，建议迁移到新版。<bk-link
                    :href="migrationUrl"
                    target="_blank"
                    theme="primary"
                >
        ...
        >
            ...
            <div class="quote-content">
                <p class="quote-text quote-title">
                    警告：关联的编译加速任务为{{ projLang() }}语言，使用了{{ quoteTitle() }}加速方案，需修改编译脚本。修改如下：
                </p>
                ...
                <template v-if="task.toolType === '3'">
                    ...
                    <p class="quote-text">
                        将配置文件bk_bazelrc和文件夹bkdistcctoolchain（含里面的文件）拷贝到项目根目录下。<a
                            :href="linkUrl"
                            target="_blank"
                            class="bk-text-primary"
                        >点击下载模板文件>></a>
