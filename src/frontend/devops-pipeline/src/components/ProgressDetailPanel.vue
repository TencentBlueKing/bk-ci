            ...
            <template>
                ...
                >
                    ...
                    <div class="progress-overview-meta">
                        ...
                        <span v-if="overviewSummary">{{ $t('progressDetail.completed') }}</span>
                    </div>
                    <div class="progress-overview-value">
                        ...
                        <span
                            ...
                            class="summary-value"
                        >
                            <em>{{ overviewSummary.current }}</em>
                            <span> / {{ overviewSummary.total }}</span>
                        </span>
                    </div>
                    <bk-progress
                        :theme="progressTheme"
                ...
                >
                    <div class="progress-section-head">
                        <span
                            class="progress-section-title"
                            :title="subtasksInfo.title || $t('progressDetail.subtasks')"
                        >{{ subtasksInfo.title || $t('progressDetail.subtasks') }}</span>
                        <span
                            v-if="subtaskSummary"
                            class="progress-section-summary"
                        >
                            {{ $t('progressDetail.completed') }}：
                            <b>{{ subtaskSummary.current }}</b>
                            <span> / {{ subtaskSummary.total }}</span>
                        </span>
                    </div>
    ...
    export default defineComponent({
        ...
        setup (props, { emit }) {
            ...
            const getSummaryParts = (summary) => {
                ...
                return {
                    current: formatCount(match[1]),
                    total: formatCount(match[2])
                }
            }
            ...
            const overviewSummary = computed(() => getSummaryParts(progressInfo.value?.summary))
            const subtaskSummary = computed(() => getSummaryParts(subtasksInfo.value?.summary))
            ...
            const buildSectionTitle = (rawTitle, label) => {
                const prefix = rawTitle || progressData.value?.taskName || ''
                if (!prefix) return label
                return `${prefix} - ${label}`
            }

            const progressTitle = computed(() => buildSectionTitle(progressInfo.value?.title, proxy.$t('progressDetail.totalProgress')))
            const timelineTitle = computed(() => buildSectionTitle(timelineInfo.value?.title, proxy.$t('progressDetail.timeline')))
            ...
            const formatDurationValue = (duration) => {
                const text = convertMStoString(duration)
                const totalSeconds = Math.floor(duration / 1000)
                if (totalSeconds >= 60 && totalSeconds % 60 === 0) {
                    const zeroSeconds = `0${proxy.$t('timeMap.seconds')}`
                    if (text.endsWith(zeroSeconds)) {
                        return text.slice(0, -zeroSeconds.length)
                    }
                }
                return text
            }
            ...
            const formatDuration = (duration) => {
                ...
                return `${proxy.$t('progressDetail.duration')} ${formatDurationValue(duration)}`
            }
