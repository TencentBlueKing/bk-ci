<template>
    <section>
        <p
            class="code-check-tip"
            v-if="!isInDetailPage"
        >
            <i class="bk-icon icon-info-circle"></i>{{ $t('store.修复代码检查发现的问题后，请重新构建并测试插件') }}
        </p>
        <section class="code-ckeck-status">
            ...
            <section class="code-check-summary">
                ...
                >
                    {{ $t('store.最近检查') }}:<span
                        class="link-txt"
                        @click="goToLink(repoUrl)"
                    >{{ commitId | commitFilter }}</span>{{ lastAnalysisTime | timeFilter }} <span
                        class="link-txt"
                        @click="goToLink(codeccUrl)"
                    >{{ $t('store.查看详情') }}</span>
                </h5>
            </section>
            <bk-button
