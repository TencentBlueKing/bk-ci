<template>
    ...
    >
        <section class="summary-tab">
            ...
            <p
                class="g-empty summary-empty"
                v-if="!detail.description"
            >
                {{ $t('store.发布者很懒，什么都没留下！') }}
            </p>
        </section>
        ...
        <section class="detail-tab">
            ...
            <section class="rate-group">
                <h3 class="rate-title">
                    <animated-integer
                        ...
                        digits="1"
                    ></animated-integer><span>{{ $t('store.共N份评分', [detail.totalNum]) }}</span>
                </h3>
                <hgroup class="rate-card">
                    <h3
                        class="rate-info"
                        v-for="(scoreItem, index) in detail.scoreItemList"
                        :key="index"
                    >
                        <comment-rate
                            :rate="scoreItem.score"
                            :width="10"
                            :height="11"
                        ></comment-rate>
                        <p class="rate-bar">
                            <span
                                class="dark-gray"
                                :style="{ flex: scoreItem.num }"
                            ></span>
                            <span
                                class="gray"
                                :style="{ flex: (+detail.totalNum > 0) ? detail.totalNum - scoreItem.num : 1 }"
                            ></span>
                        </p>
            ...
            <p
                class="comments-more"
                v-if="!isLoadEnd && commentList.length > 0"
                @click="getComments(true)"
            >
                {{ $t('store.阅读更多内容') }}
            </p>
            <p
                class="g-empty comment-empty"
                v-if="commentList.length <= 0"
            >
                {{ $t('store.空空如洗，快来评论一下吧！') }}
            </p>
        </section>
        <transition name="atom-fade">
            <commentDialog
                ...
                @closeDialog="showComment = false"
                :name="detail.name"
                :code="detailCode"
                :id="detail.detailId"
                :comment-id="(detail.userCommentInfo || {}).commentId"
            ></commentDialog>
        </transition>
    </section>
</template>
