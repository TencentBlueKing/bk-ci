<template>
    <main>
        <div class="content-header">
            <div class="atom-total-row">
                <button class="bk-button bk-primary" @click="relateImage">
                    <span style="margin-left: 0;">关联镜像</span>
                </button>
            </div>
        ...
        >
            <bk-table-column label="镜像名称">
                <template slot-scope="props">
                    <span class="atom-name" :title="props.row.imageName" @click="goToImageDetail(props.row.imageCode)">{{ props.row.imageName }}</span>
                </template>
            </bk-table-column>
            <bk-table-column label="镜像来源" prop="imageSourceType" :formatter="sourceTypeFormatter"></bk-table-column>
            <bk-table-column label="镜像地址" prop="imageRepoUrl">
                <template slot-scope="props">
                    <span>{{ props.row.imageRepoUrl || '-' }}</span>
                </template>
            </bk-table-column>
            <bk-table-column label="大小" prop="imageSize">
                <template slot-scope="props">
                    <span>{{ props.row.imageSize || '-' }}</span>
                </template>
            </bk-table-column>
            <bk-table-column label="Tag" prop="imageTag">
                <template slot-scope="props">
            ...
            <bk-table-column label="状态">
                <template slot-scope="props">
                    ...
                    <span>{{ imageStatusList[props.row.imageStatus] }}</span>
                </template>
            </bk-table-column>
            ...
            <bk-table-column label="操作" width="250" class-name="handler-btn">
                <template slot-scope="props">
                    <span class="shelf-btn"
                        ...
                        @click="$router.push({ name: 'editImage', params: { imageId: props.row.imageId } })">上架
                    </span>
                    <span class="shelf-btn"
                        v-if="props.row.imageStatus === 'RELEASED'"
                        @click="$router.push({ name: 'editImage', params: { imageId: props.row.imageId } })">升级
                    </span>
                    <span class="shelf-btn"
                        v-if="props.row.imageStatus === 'RELEASED'"
                        @click="$router.push({ name: 'install', query: { code: props.row.imageCode, type: 'image', from: 'atomList' } })">安装
                    </span>
                    <span class="schedule-btn"
                        v-if="['AUDITING', 'COMMITTING', 'CHECKING', 'CHECK_FAIL', 'UNDERCARRIAGING', 'TESTING'].includes(props.row.imageStatus)"
                        @click="$router.push({ name: 'imageProgress', params: { imageId: props.row.imageId } })">进度
                    </span>
                    ...
                    >下架</span>
                    <span @click="deleteImage(props.row.imageCode)" v-if="['INIT', 'GROUNDING_SUSPENSION', 'UNDERCARRIAGED'].includes(props.row.imageStatus)">删除</span>
                </template>
            </bk-table-column>
        </bk-table>
        ...
        <bk-sideslider :is-show.sync="relateImageData.show"
            ...
            <template slot="content">
                <bk-form ref="relateForm" class="relate-form" label-width="100" :model="relateImageData.form" v-bkloading="{ isLoading: relateImageData.isLoading }">
                    <bk-form-item label="镜像名称" :required="true" property="imageName" desc="镜像在研发商店中的别名" :rules="[requireRule]">
                        <bk-input v-model="relateImageData.form.imageName" placeholder="请输入镜像名称"></bk-input>
                    </bk-form-item>
                    <bk-form-item label="镜像标识" :required="true" property="imageCode" desc="镜像在研发商店中的唯一标识" :rules="[requireRule, alpRule]">
                        <bk-input v-model="relateImageData.form.imageCode" placeholder="请输入镜像标识"></bk-input>
                    </bk-form-item>
                    <bk-form-item label="镜像源" :required="true" property="imageSourceType" class="h32" :rules="[requireRule]">
                    ...
                    <bk-form-item label="调试项目" :required="true" property="projectCode" desc="在发布过程中，可以在该项目下调试镜像" :rules="[requireRule]">
                        <bk-select v-model="relateImageData.form.projectCode" searchable placeholder="请选择项目" @change="toggleProjectList">
                    ...
                    <bk-form-item label="凭证" property="ticketId" :rules="[requireRule]" desc="若为私有镜像，请提供凭证，用于流水线执行时拉取镜像" v-if="relateImageData.form.imageSourceType === 'THIRD'">
                        <bk-select v-model="relateImageData.form.ticketId" searchable placeholder="请选择凭证" :loading="relateImageData.isLoadingTicketList">
                    ...
                    <bk-form-item>
                        <bk-button theme="primary" @click.native="submitRelateImage">提交</bk-button>
                        <bk-button @click.native="cancelRelateImage">取消</bk-button>
                    </bk-form-item>
                </bk-form>
            </template>
        </bk-sideslider>
        ...
        <bk-sideslider :is-show.sync="offlineImageData.show"
            ...
            <template slot="content">
                <bk-form ref="offlineForm" class="relate-form" label-width="100" :model="offlineImageData.form" v-bkloading="{ isLoading: offlineImageData.isLoading }">
                    <bk-form-item label="镜像名称" property="imageName">
                        <span class="lh30">{{offlineImageData.form.imageName}}</span>
                    </bk-form-item>
                    <bk-form-item label="镜像标识" property="imageCode">
                        <span class="lh30">{{offlineImageData.form.imageCode}}</span>
                    </bk-form-item>
                    <bk-form-item label="镜像版本" property="version">
                        <bk-select v-model="offlineImageData.form.version" searchable placeholder="请选择镜像版本">
                    ...
                    <bk-form-item label="下架原因" :required="true" property="reason" :rules="[requireRule]">
                        <bk-input type="textarea" v-model="offlineImageData.form.reason" placeholder="请输入下架原因"></bk-input>
                    </bk-form-item>
                    <bk-form-item>
                        <bk-button theme="primary" @click.native="submitOfflineImage">提交</bk-button>
                        <bk-button @click.native="cancelOfflineImage">取消</bk-button>
                    </bk-form-item>
                </bk-form>
            </template>
        </bk-sideslider>
    ...
    export default {
        ...
        methods: {
            submitOfflineImage (row) {
                ...
                }).catch(() => this.$bkMessage({ message: '校验失败，请修改再试', theme: 'error' }))
            },
            ...
            submitRelateImage () {
                this.$refs.relateForm.validate((val) => {
                    if (val) {
                        ...
                        this.$store.dispatch('store/requestRelImage', postData).then((res) => {
                            if (res === true) {
                                this.relateImageData.isLoading = false
                                this.goToImageDetail(this.relateImageData.form.imageCode)
                            } else {
                                this.$bkMessage({ message: res.message || '关联镜像失败，请稍后再试', theme: 'error' })
                            }
                        }).catch((err) => {
                            this.$bkMessage({ message: err.message || err, theme: 'error' })
                    ...
                    this.$bkMessage({ message: '校验不通过，请修改后再试', theme: 'error' })
                })
            },
