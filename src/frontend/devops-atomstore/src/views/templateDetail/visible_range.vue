    ...
    export default {
        ...
        data () {
            return {
                ...
                emptyTipsConfig: {
                    title: '暂时没有设置可见范围',
                    desc: '未设置可见对象时，仅模板成员可以安装到名下项目中使用。设置可见对象后，对应用户可以在Store中查看并安装使用。',
                    btns: [
                        {
                            type: 'primary',
                            size: 'normal',
        ...
        methods: {
            ...
            async requestList () {
                try {
                    const res = await this.$store.dispatch('store/requesttplVisibleList', {
                ...
                if (!target.length) {
                    ...
                    this.$bkInfo({
                        ...
                        confirmFn: async () => {
                            ...
                            this.deleteTplVisiable(deptIds)
                        }
                    })
                }
            },
            ...
            async deleteTplVisiable (deptIds) {
                let message, theme
                try {
                    await this.$store.dispatch('store/deleteTplVisiable', {
                        templateCode: this.templateCode,
                        deptIds
                    })
