<template>
    <div class="atom-information-wrapper">
        <div class="inner-header">
            <div class="title"> {{ $t('store.插件详情') }} </div>
            <span @click="goToEditAtom" :class="[{ 'disable': !showEdit }, 'header-edit']" :title="!showEdit && $t('store.只有处于审核驳回、已发布、上架中止和已下架的状态才允许修改基本信息')"> {{ $t('store.编辑') }} </span>
        </div>
