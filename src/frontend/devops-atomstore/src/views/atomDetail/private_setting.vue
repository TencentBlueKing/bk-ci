<template>
    <article class="setting-private">
        <h3 class="private-title"> {{ $t('store.私有配置') }} </h3>

        <section class="private-main" v-bkloading="{ isLoading }" v-if="privateList.length">
            <bk-button theme="primary" :title="$t('store.新增配置')" @click="showAdd = true"> {{ $t('store.新增配置') }} </bk-button>
        ...
        <section class="g-empty private-empty" v-else> {{ $t('store.暂无私有配置') }} <p> {{ $t('store.插件级别的敏感配置信息，可以在此添加，不暴露在插件代码中。插件逻辑中用到时从输入获取即可') }} </p>
            <button @click="showAdd = true"> {{ $t('store.添加') }} </button>
        </section>
