    ...
    <article v-bkloading="{ isLoading }" class="image-progress-home">
        <h3 class="market-home-title">
            ...
            <p class="title-name">
                ...
                <span class="banner-des back-home" @click="toImageList">工作台</span>
                <i class="right-arrow banner-arrow"></i>
                <span class="banner-des">上架镜像({{imageDetail.imageName}})</span>
            </p>
            <a class="title-work" target="_blank" href="http://iwiki.oa.com/pages/viewpage.action?pageId=22118721">镜像指引</a>
        </h3>
        <main v-if="!isLoading" class="image-progress-main">
            <section class="image-progress-section">
                <h3>
                    <span>发布进度</span>
                    <span @click="cancelRelease" :class="[{ disable: !permission }, 'cancel-release']" :title="permissionMsg">取消发布</span>
                </h3>
                        ...
                        <div class="step-card" v-for="(entry, index) in progressStatus" :key="index"
                            ...
                            <div class="retry-bth">
                                <span :class="[{ disable: !permission }, 'rebuild-btn']"
                                    ...
                                    @click.stop="reCheck"
                                >
                                    重新验证<i class="col-line"></i>
                                </span>
                                <span class="log-btn" v-if="entry.code === 'check' && entry.status !== 'undo'" @click.stop="readLog">日志</span>
                                <span class="test-btn" v-if="entry.code === 'test' && entry.status === 'doing'">
                                    <a target="_blank" :href="`/console/pipeline/${imageDetail.projectCode}/list`">测试</a>
                                </span>
                            </div>
            ...
            <div class="released-tips" v-else>
                ...
                <div class="handle-btn">
                    <bk-button class="bk-button bk-primary" size="small" @click.native="toImageList">工作台</bk-button>
                    <bk-button class="bk-button bk-default" size="small" @click.native="toAtomStore">研发商店</bk-button>
                </div>
            </div>
        </main>
        <bk-sideslider
...
<script>
    ...
    export default {
        ...
        computed: {
            permissionMsg () {
                let str = ''
                if (!this.permission) str = '只有镜像管理员或当前流程创建者可以操作'
                return str
            },
        ...
        methods: {
            ...
            reCheck () {
                ...
                this.requestRecheckImage(imageId).then(() => {
                    this.$bkMessage({ message: '发起重新验证成功', theme: 'success' })
            ...
            cancelRelease () {
                ...
                const confirmFn = () => {
                    ...
                    this.requestCancelRelease(imageId).then(() => {
                        this.$bkMessage({ message: '取消发布成功', theme: 'success' })
                        this.toImageList()
                    }).catch((err) => {
                        this.$bkMessage({ message: err.message || err, theme: 'error' })
