<template>
    <div class="visible-range-wrapper">
        ...
        <section class="sub-view-port" v-bkloading="{ isLoading }">
            <div class="visible-range-content" v-if="!isLoading && visibleList.length">
                <div class="handle-row" v-if="userInfo.isProjectAdmin">
                    <button class="bk-button bk-primary add-button" type="button" @click="addHandle()">添加</button>
                    <button class="bk-button bk-default" type="button" @click="bitchrRemove()">批量删除</button>
                </div>
                <bk-table style="margin-top: 15px;"
                    empty-text="未设置可见对象时，仅插件成员可以安装到名下项目中使用。设置可见对象后，对应用户可以在Store中查看并安装使用。"
                    :data="visibleList"
                    @select="select"
                    @select-all="selectAll"
                >
                    <bk-table-column type="selection" width="70" align="center"></bk-table-column>
                    <bk-table-column label="可见对象" prop="deptName"></bk-table-column>
                    <bk-table-column label="状态">
                        <template slot-scope="props">
                            <span>{{ statusMap[props.row.status] }}</span>
                            <span class="audit-tips" v-if="props.row.status === 'APPROVING'"><i class="bk-icon icon-info-circle"></i>由蓝盾管理员审核</span>
            ...
            <empty-tips v-if="!isLoading && !visibleList.length"
    ...
    export default {
        ...
        computed: {
            ...
            emptyTipsConfig () {
                return {
                    title: '暂时没有设置可见范围',
                    desc: '未设置可见对象时，仅镜像成员可以安装到名下项目中使用。设置可见对象后，对应用户可以在Store中查看并安装使用。',
                    btns: [
                        {
                            type: 'primary',
                            size: 'normal',
        ...
        methods: {
            async requestList () {
                this.isLoading = true
                try {
                    const res = await this.$store.dispatch('store/requestImageVisableList', this.imageCode)
            ...
            async requestDeleteVisiable (deptIds) {
                let message, theme
                try {
                    await this.$store.dispatch('store/requestDeleteImageVis', {
