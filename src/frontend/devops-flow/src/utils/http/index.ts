...
import { createDebouncedPromise, createThrottledPromise } from './requestControl'
...
export async function request<T = unknown>(
  ...
  const cacheKey =
    meta.cacheKey ||
    `${method}:${url}:${JSON.stringify(finalConfig.params || finalConfig.data || {})}`
...
export function del<T>(url: string, config?: HttpRequestConfig): Promise<T> {
  return request<T>('DELETE', url, config)
}

export function head<T = unknown>(url: string, config?: HttpRequestConfig): Promise<T> {
