...
<script>
    ...
    export default {
        ...
        setup () {
            ...
            // 获取部署任务列表
            const fetchDeployTaskList = async (params) => {
                try {
                    // 使用真实接口
                    // const res = await proxy.$store.dispatch('environment/requestDeployTaskList', {
                    ...
                    console.error('获取部署任务列表失败:', err)
                    throw err
                }
            }
            
            // 获取部署任务详情
            const fetchDeployTaskDetail = async (taskId, params) => {
                try {
                    // 使用真实接口
                    // const res = await proxy.$store.dispatch('environment/requestDeployTaskDetail', {
                    //     projectId: projectId.value,
                    //     envHashId: envHashId.value,
                    //     taskId,
                    //     params
