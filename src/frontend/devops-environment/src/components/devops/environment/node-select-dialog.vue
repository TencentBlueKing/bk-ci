<template>
    <bk-dialog v-model="nodeSelectConf.isShow" ext-cls="node-select-wrapper" :width="'900'" :close-icon="false">
        <div v-bkloading="{
                        ...
                        <div class="biz-ip-searcher-wrapper">
                            <div class="biz-searcher" @click="focusSearch" ref="bizSearcher">
                                <ul class="search-key" ref="searchKey">
                                    ...
                                    <li class="input-item">
                                        <input type="text" class="search-input" ref="searchInput" v-model="inputValue" :style="inputStyle"
                                            @blur="handleBlur" @paste="paste" @keyup="keyupHandler">
                                    </li>
                                </ul>
                            </div>
                            <div class="actions">
        ...
        <div slot="footer">
            <div class="footer-handler">
                <bk-button theme="primary" @click="confirmFn" :disabled="nodeSelectConf.unselected">{{ nodeSelectConf.importText
                }}</bk-button>
...
<script>
    export default {
        ...
        methods: {
            ...
            isCurrentUser (row) {
                const { curUserInfo } = this
                return [row.operator, row.bakOperator].includes(curUserInfo.username)
            },
            isShowOperateChange (row) {
                const CC = 'CC'
                const CMDB = 'CMDB'
                const ccOrcmdb = [CC, CMDB].includes(row.nodeType)
                let bakOperator = []
                if (row.nodeType === CMDB) {
                    bakOperator = row.bakOperator.split(';')
                } else if (row.nodeType === CC) {
                    bakOperator = [row.bakOperator]
                }
                return ccOrcmdb && ![row.operator, ...bakOperator].includes(row.createdUser)
            },
            handleFocus () {
