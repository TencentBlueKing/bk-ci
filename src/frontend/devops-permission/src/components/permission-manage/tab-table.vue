  ...
  <bk-loading :loading="loading" :zIndex="100">
    ...
    >
      ...
      <bk-table-column :label="t('状态')" prop="beingHandedOver" v-if="isShowOperation">
        <template #default="{row}">
          <span @click="handleToHandoverPage(row.flowNo)" :class="row.flowNo ? 'text-blue' : ''">
...
function handleToResourcePage(row) {
  ...
  const url = URL_TEMPLATES[row.resourceType]?.(groupTableStore.projectId, row);
  if (url) {
    window.open(url);
  }
}
function handleToHandoverPage(flowNo) {
  if (flowNo) {
    window.open(`${window.location.origin}/${window.getRoutePrefix()}/permission/my-handover?flowNo=${flowNo}&type=handoverFromMe`)
  }
}
</script>
