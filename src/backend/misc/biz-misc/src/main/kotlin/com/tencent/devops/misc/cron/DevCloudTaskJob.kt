...
class DevCloudTaskJob @Autowired constructor(
    ...
    fun run() {
        ...
        try {
            ...
            if (lockSuccess) {
                ...
                val lock = redisOperation.get(jobExecuteKey())
                if (lock == null) {
                    redisOperation.set(jobExecuteKey(), "LOCKED", delay)
    ...
    private fun createDevCloudVm(task: TDevCloudTaskRecord) {
        ...
        for (i in 0 until task.replica) {
            val devCloudTaskId: String
            try {
                devCloudTaskId = devCloudClient.createContainer(task.operator, DevCloudContainer(
                        task.containerName,
                        task.containerType,
                        task.image,
            ...
            devCloudTaskDao.updateDevCloudTaskId(dslContext, task.taskId, devCloudTaskId)
            ...
            if (createResult.first == TaskStatus.SUCCEEDED) {
                ...
                nodeAuthUtils.createNodeResource(task.operator, task.projectId, allDevCloudNodes[i].nodeId, allDevCloudNodes[i].nodeStringId, allDevCloudNodes[i].nodeIp)
            } else {
                // 创建失败，记录日志，告警，通知
        ...
        if (errorMsg.isEmpty()) {
            ...
            val (title, content) = createAddVmEmailBody(task.replica, 0, allDevCloudNodes, task.password, emptyMap())
            sendEmail(task.operator, task.projectId, title, content)
        } else {
            // 有失败或全部失败
            ...
            val (title, content) = createAddVmEmailBody(task.replica, errorMsg.size, allDevCloudNodes, task.password, errorMsg)
            sendEmail(task.operator, task.projectId, title, content)
        }
    }
    ...
    private fun buildImage(task: TDevCloudTaskRecord) {
        ...
        try {
            val devCloudImage = DevCloudImage(
                    ...
                    ImageParams(Container(task.containerName), Registry(task.registryHost, task.registryUser, task.registryPwd))
            )
        ...
        if (createResult.first == TaskStatus.SUCCEEDED) {
            ...
            devCloudTaskDao.updateTaskStatus(dslContext, task.taskId, createResult.first) // 刷新状态为成功
            val properties = mapOf("devops.creator" to task.operator,
    ...
    private fun copyImageToDockerHost(operator: String, projectId: String, imageName: String, imageTag: String) {
        try {
            client.get(ServiceImageResource::class).setBuildImage(operator, projectId, imageName, imageTag)
    ...
    private fun createAddVmEmailBody(successNum: Int, failedNum: Int, devCloudNodes: List<TNodeRecord>, password: String, errMsg: Map<Long, String>): Pair<String, String> {
        ...
        devCloudNodes.forEach { node ->
            sb.append("""
                                            <tr>
                                                ...
                                                <td style=" padding: 16px; border: 1px solid #e6e6e6;text-align: left; font-weight: normal;">${NodeStatus.parseByName(node.nodeStatus).statusName}</td>
                                                <td style=" padding: 16px; border: 1px solid #e6e6e6;text-align: left; font-weight: normal;">$password</td>
                                                <td style=" padding: 16px; border: 1px solid #e6e6e6;text-align: left; font-weight: normal;">${errMsg[node.nodeId] ?: ""}</td>
                                            </tr>
                    """)
        }
        sb.append("""
    ...
    private fun createBuildImageEmailBody(success: Boolean, imageNameTag: String, desc: String, msg: String): Pair<String, String> {
        val sb = StringBuilder()
        if (success) {
            sb.append("制作镜像成功！<br/>")
    ...
    private fun sendEmail(userId: String, projectId: String, titleStr: String, contentStr: String) {
        try {
            ...
            logger.info("Send email, projectName: ${bkAuthProject.project_name}")
            val templateParams = mapOf(
                    "templateTitle" to titleStr,
                    "templateContent" to content,
                    "projectName" to (bkAuthProject.project_name),
                    "logoUrl" to logoUrl,
