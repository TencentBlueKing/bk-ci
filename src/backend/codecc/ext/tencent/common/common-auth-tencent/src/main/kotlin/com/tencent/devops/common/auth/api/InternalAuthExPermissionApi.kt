...
import com.tencent.devops.common.auth.api.pojo.external.AuthExPropertiesData
...
import com.tencent.devops.common.auth.pojo.external.request.InternalAuthExBatchAuthorizedUserRequest
import com.tencent.devops.common.auth.pojo.external.request.InternalAuthExBatchPermissionVerityRequest
import com.tencent.devops.common.auth.pojo.external.request.InternalAuthExPolicyResourceType
import com.tencent.devops.common.auth.pojo.external.request.InternalAuthExResourceListRequest
...
class InternalAuthExPermissionApi(
    ...
    override fun validateGongfengPermission(
        ...
        val authTaskService = SpringContextUtil.getBean(GongfengAuthTaskService::class.java)
        //获取工蜂基本信息
        val gongfengBaseInfo = if (Pattern.compile("^git_[0-9]+$").matcher(projectId).find()) {
            // 工蜂CI项目的工蜂ID就是项目ID里 git_ 之后的内容
            val gongfengId = projectId.replace("git_", "").toInt()
            logger.info("auth gongfeng ci by gongfeng id {}", gongfengId)
            authTaskService.getGongfengCIProjInfo(gongfengId) ?: return false
        } else {
            authTaskService.getGongfengProjInfo(taskId.toLong()) ?: return false
        }
        ...
        // 如果工蜂ID是-1，则代表此项目是私有项目，不需要通过工蜂公共项目鉴权接口
        ...
        val result: CodeCCResult<List<GitMember>>
        // 校验工蜂项目权限
        try {
            val data = client.getDevopsService(ExternalCodeccRepoResource::class.java).getRepoAllMembers(gongfengBaseInfo.httpUrl, user).data
            result = CodeCCResult(data ?: listOf())
        } catch (e : Exception) {
            logger.error("", e)
            return false
        }
        ...
        // Oauth 权限过期
        if (result.status.toString() == CommonMessageCode.OAUTH_TOKEN_IS_INVALID) {
    ...
    private fun queryResourceListForUser(
        ...
        val result: AuthExResponse<Set<String>> = queryResourceList(
                projectCode = projectCode,
                policyCodes = actions,
                resourceType = resourceType,
                serviceCode = serviceCode,
