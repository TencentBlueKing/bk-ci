...
open class GongfengMessagListenerConfig {
    ...
    open fun createTaskListenerContainer(
        ...
        //确保只有一个消费者消费，保证负载不超时
        val adapter = MessageListenerAdapter(gongfengCreateTaskListener, gongfengCreateTaskListener::executeCreateTask.name)
    ...
    open fun updateGongfengListenerContainer(
        ...
        container.setRabbitAdmin(rabbitAdmin)
        //确保只有一个消费者消费，保证负载不超时
        val adapter = MessageListenerAdapter(gongfengCreateTaskListener, gongfengCreateTaskListener::updateGongfengStatInfo.name)
        adapter.setMessageConverter(messageConverter)
        container.messageListener = adapter
        return container
    }
    ...
    open fun triggerPipelineListenerContainer(
        ...
        val adapter = MessageListenerAdapter(gongfengCreateTaskListener, gongfengCreateTaskListener::executeTriggerPipeline.name)
        adapter.setMessageConverter(messageConverter)
        container.messageListener = adapter
        return container
    }
    ...
    open fun createActiveProjListenerContainer(
        ...
        container.setConsecutiveActiveTrigger(1)
        container.setRabbitAdmin(rabbitAdmin)
        val adapter = MessageListenerAdapter(gongfengCreateTaskListener, gongfengCreateTaskListener::executeActiveProjectTask.name)
        adapter.setMessageConverter(messageConverter)
        container.messageListener = adapter
        return container
    }
    ...
    open fun newTaskRetryListenerContainer(
        ...
        container.setMaxConcurrentConsumers(1)
        container.setRabbitAdmin(rabbitAdmin)
        val adapter = MessageListenerAdapter(gongfengFailRecordListener, gongfengFailRecordListener::openSourceFailTaskRetry.name)
        adapter.setMessageConverter(messageConverter)
        container.messageListener = adapter
        return container
    }
