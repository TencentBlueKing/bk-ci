...
import com.tencent.bk.codecc.codeccjob.dao.mongorepository.*
...
class CodeScoringConsumer @Autowired constructor(
    ...
    private val codeccDomain: String? = null
    ...
    fun scoring() {
        ...
        try {
            ...
            val res = client.get(ServiceGongfengTaskRestResource::class.java).bkPluginTaskIds
                ...
                try {
                    ...
                    if (rdIndicatorsScore < 85.26) {
                        ...
                        if (codeSecurityScore < 95) {
                            val defectList = lintDefectV2Repository.findByTaskIdAndToolNameAndStatus(taskId, "WOODPECKER-SENSITIVE", 1)
                            authorSet.addAll(defectList.stream().map(LintDefectV2Entity::getAuthor).collect(Collectors.toSet()))
                        }
                        ...
                        if (authorSet.isNotEmpty()) {
                            val bodyParams = mapOf(
                                    ...
                                    "codeccDetailUrl" to "http://$codeccDomain/codecc/codecc/task/$taskId/detail",
    ...
    private fun getDefectNum(toolName: String, taskId: Long, buildId: String): Pair<Double, Double> {
        ...
        when (toolName) {
            ...
            "CLOC" -> {
                val res = clocStatisticRepository.findByTaskIdAndBuildId(taskId, buildId)
                var totalLine = 0L
                var language = -1
                res.forEach {
    ...
    fun sendWeChat(unqualifiedList: MutableList<MetricsEntity>) {
        val taskIds = unqualifiedList.stream()
                ...
                .collect(Collectors.toList())
        val result = client.get(ServiceGongfengTaskRestResource::class.java).getGongfengProjInfoByTaskId(taskIds)
