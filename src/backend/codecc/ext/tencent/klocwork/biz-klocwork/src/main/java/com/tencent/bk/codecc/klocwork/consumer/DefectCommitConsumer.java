...
public class DefectCommitConsumer
{
     ...
     */
    @RabbitListener(bindings = @QueueBinding(key = ROUTE_DEFECT_COMMIT_KLOCWORK,
            value = @Queue(value = QUEUE_DEFECT_COMMIT_KLOCWORK, durable = "true"),
            exchange = @Exchange(value = EXCHANGE_DEFECT_COMMIT_KLOCWORK, durable = "true", delayed = "true", type = "topic")))
    public void commitDefect(CommitDefectVO commitDefectVO)
    {
        try
        {
            ...
            try
            {
                kwDefectService.commitDefect(commitDefectVO);
            }
            catch (Exception e)
            {
