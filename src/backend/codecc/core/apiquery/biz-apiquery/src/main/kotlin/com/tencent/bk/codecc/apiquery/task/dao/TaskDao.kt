...
class TaskDao @Autowired constructor(
    ...
    fun findByBgId(
        ...
        val match = Aggregation.match(
            Criteria.where("bg_id").`is`(bgId).and("create_from").`is`("gongfeng_scan").and("custom_proj_info").exists(false).and("project_id").ne("CUSTOMPROJ_TEG_CUSTOMIZED")
        )
        val lookup =
    ...
    fun findTaskInfoPage(reqReq: TaskToolInfoReqVO, pageable: Pageable): Page<TaskInfoModel> {
        ...
        // 指定查询字段
        val project = Aggregation.project("task_id", "name_en", "name_cn", "code_lang",
        ...
        // 允许磁盘操作(支持较大数据集合的处理)
        val options = AggregationOptions.Builder().allowDiskUse(true).build()
        val aggregation = Aggregation.newAggregation(Aggregation.match(criteria), sort, skip, project, limit).withOptions(options)
