...
public class ToolDao
{
    ...
    private Criteria getCriteriaByCondition(Collection<Long> taskIds, Collection<String> tools, Integer followStatus,
        ...
        if (followStatus != null) {
            // 下架取反 = 非下架, 有效的
            if (isNot && ComConstants.FOLLOW_STATUS.WITHDRAW.value() == followStatus) {
    ...
    public Page<ToolConfigInfoModel> findToolInfoPage(TaskToolInfoReqVO reqVO, Pageable pageable) {
        ...
        if (followStatus != null) {
            // 除了 已接入和下架 其他都是未跟进
            if (followStatus == 0) {
                List<Integer> status = Lists.newArrayList(ComConstants.FOLLOW_STATUS.NOT_FOLLOW_UP_0.value(),
                        ComConstants.FOLLOW_STATUS.NOT_FOLLOW_UP_1.value(),
                        ComConstants.FOLLOW_STATUS.EXPERIENCE.value(), ComConstants.FOLLOW_STATUS.ACCESSING.value(),
                        ComConstants.FOLLOW_STATUS.HANG_UP.value());
