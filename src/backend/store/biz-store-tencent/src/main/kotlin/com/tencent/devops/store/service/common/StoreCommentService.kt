...
import com.tencent.devops.model.atom.tables.records.TStoreCommentRecord
 ...
 */
@Service
class StoreCommentService @Autowired constructor(
    ...
    private val logger = LoggerFactory.getLogger(StoreCommentService::class.java)
    ...
    fun addStoreComment(userId: String, storeId: String, storeCode: String, storeCommentRequest: StoreCommentRequest, storeType: StoreTypeEnum): Result<StoreCommentInfo?> {
        ...
        sendRtx(storeCode, storeType, storeId, userId, storeCommentRequest)

        return getStoreComment(userId, commentId)
    }

    private fun sendRtx(storeCode: String, storeType: StoreTypeEnum, storeId: String, userId: String, storeCommentRequest: StoreCommentRequest) {
        // 通知插件管理员和开发人员、平台管理员

        val memberData = storeMemberService.batchListMember(listOf(storeCode), storeType).data
        val members = memberData?.get(storeCode)
        val receivers = if (members == null) {
            commentNotifyAdmin.split(";").toSet()
        } else {
            commentNotifyAdmin.split(";").toSet().plus(members.toSet())
        }
        ...
        val url = "${HomeHostUtil.innerServerHost()}/console/store/atomStore/detail/${storeType.name.toLowerCase()}/$storeCode"
        val storeName = storeCommonService.getStoreNameById(storeId, storeType)
        val rtxMessage = RtxUtil.makeCommentNotifyMessage(userId, storeName, storeCommentRequest, url, receivers)
    ...
    fun updateStoreComment(userId: String, commentId: String, storeCommentRequest: StoreCommentRequest): Result<Boolean> {
        ...
        val storeTypeObj = StoreTypeEnum.getStoreTypeObj(storeCommentRecord.storeType.toInt())
        sendRtx(storeCommentRecord.storeCode, storeTypeObj!!, storeCommentRecord.storeId, userId, storeCommentRequest)
