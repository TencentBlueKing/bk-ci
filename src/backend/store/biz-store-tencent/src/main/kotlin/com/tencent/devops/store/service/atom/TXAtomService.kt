 ...
 */
@Service
class TXAtomService @Autowired constructor(
    private val dslContext: DSLContext,
    private val atomDao: AtomDao,
    private val atomLabelRelDao: AtomLabelRelDao,
    private val atomClassifyDao: ClassifyDao,
    ...
    private val pieplineAuthServiceCode: BSPipelineAuthServiceCode
): AtomService {
    ...
    private val logger = LoggerFactory.getLogger(TXAtomService::class.java)
    ...
    fun getOpPipelineAtoms(
        ...
        logger.info("the atomName is :$atomName,serviceScope is :$serviceScope,os is :$os,atomType is :$atomType,category is :$category,classifyId is :$classifyId,page is :$page,pageSize is :$pageSize")
        val pipelineAtomList = atomDao.getOpPipelineAtoms(dslContext, atomName, atomType, serviceScope, os, category, classifyId, atomStatus, sortType?.sortType, desc, page, pageSize).map {
    ...
    override fun savePipelineAtom(userId: String, atomRequest: AtomCreateRequest): Result<Boolean> {
        ...
        // 校验插件分类是否合法
        atomClassifyDao.getClassify(dslContext, atomRequest.classifyId) ?: return MessageCodeUtil.generateResponseDataObject(CommonMessageCode.PARAMETER_IS_INVALID, arrayOf(atomRequest.classifyId), false)
    ...
    override fun updatePipelineAtom(userId: String, id: String, atomUpdateRequest: AtomUpdateRequest): Result<Boolean> {
        ...
        // 校验插件分类是否合法
        atomClassifyDao.getClassify(dslContext, atomUpdateRequest.classifyId) ?: return MessageCodeUtil.generateResponseDataObject(CommonMessageCode.PARAMETER_IS_INVALID, arrayOf(atomUpdateRequest.classifyId), false)
            ...
            dslContext.transaction { t ->
                ...
                if (null != recommendFlag) {
                    ...
                    if (null != atomFeatureRecord) {
                        ...
                        atomFeatureDao.addAtomFeature(context, userId, AtomFeatureRequest(atomCode = atomRecord.atomCode, visibilityLevel = atomRecord.visibilityLevel, recommendFlag = recommendFlag))
                    }
                }
            }
            Result(true)
    ...
    fun hasManagerPermission(projectId: String, userId: String): Boolean {
        return bkAuthProjectApi.getProjectUsers(pieplineAuthServiceCode, projectId, BkAuthGroup.MANAGER)
            .contains(userId)
    }
    ...
    fun uninstallAtom(
        ...
        // 是否还有流水线使用待卸载的插件
        val pipelineCnt = client.get(ServiceMeasurePipelineResource::class).getPipelineCountByAtomCode(atomCode, projectCode).data ?: 0
        if (pipelineCnt > 0) {
    ...
    fun updateAtomBaseInfo(userId: String, atomCode: String, atomBaseInfoUpdateRequest: AtomBaseInfoUpdateRequest): Result<Boolean> {
        ...
        if (null != visibilityLevel && visibilityLevel.level != dbVisibilityLevel) {
            ...
            try {
                updateGitRepositoryResult = client.get(ServiceRepositoryResource::class).updateGitCodeRepository(
                    userId, newestAtomRecord.repositoryHashId,
            ...
            if (updateGitRepositoryResult.isNotOk()) {
                return Result(updateGitRepositoryResult.status, updateGitRepositoryResult.message ?: "") // 工蜂更新代码信息失败则把报错信息返回给前端
            }
        }
        ...
        dslContext.transaction { t ->
            ...
            atomDao.updateAtomBaseInfo(context, userId, atomIdList, atomBaseInfoUpdateRequest)
                val atomFeatureRequest = AtomFeatureRequest(
                    atomCode = atomCode,
                    visibilityLevel = visibilityLevel?.level,
                    privateReason = atomBaseInfoUpdateRequest.privateReason
                )
                if (visibilityLevel == VisibilityLevelEnum.LOGIN_PUBLIC) {
                    atomFeatureRequest.privateReason = "" // 如果选择开源需把不开源理由置为空
                }
                atomFeatureDao.updateAtomFeature(context, userId, atomFeatureRequest)
            // 更新标签信息
