...
data class RepoCreateRequest(
    ...
    @Schema(title = "仓库类别", required = true)
    val category: RepositoryCategory,
    @Schema(title = "是否公开", required = true)
    val public: Boolean,
    @Schema(title = "简要描述", required = false)
    val description: String? = null,
    @Schema(title = "仓库配置", required = true)
    val configuration: RepositoryConfiguration? = null,
    @Schema(title = "存储凭证key", required = false)
    val storageCredentialsKey: String? = null,
    @Schema(title = "仓库配额", required = false)
    val quota: Long? = null,
    @Schema(title = "是否展示", required = false)
    val display: Boolean = true,
    @Schema(title = "操作用户", required = false)
