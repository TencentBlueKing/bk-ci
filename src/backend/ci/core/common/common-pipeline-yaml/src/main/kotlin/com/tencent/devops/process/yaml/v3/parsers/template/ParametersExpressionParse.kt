...
import com.tencent.devops.process.yaml.v3.exception.YamlTemplateException
...
import com.tencent.devops.process.yaml.v3.parsers.template.models.ExpressionBlock
            ...
            if (param.name.contains(".")) {
                throw error(
                    Constants.PARAMETER_FORMAT_ERROR.format(
                        path.toString(),
                        "parameter name ${param.name} not allow contains '.'"
                    )
                )
            }
    ...
    fun fromJsonToArrayContext(path: TemplatePath, parameterName: String, value: Iterable<*>): ArrayContextData {
        ...
        if (!jsonTree.isArray) {
            throw error(
                Constants.PARAMETER_FORMAT_ERROR.format(
                    path.toString(),
                    "array parameter $parameterName value  [$value] json type [${jsonTree.nodeType}] not array."
                )
            )
        }
