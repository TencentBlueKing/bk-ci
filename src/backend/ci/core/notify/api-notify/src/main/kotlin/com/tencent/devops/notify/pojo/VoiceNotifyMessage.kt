...
open class VoiceNotifyMessage : BaseMessage() {
    @Schema(title = "接收人(英文ID)，支持多个")
    var receivers = mutableSetOf<String>()

    @Schema(title = "任务名称，不超过200字符")
    var taskName = ""

    @Schema(title = "呼叫内容，建议只传简短的文字内容，详细信息通过企业微信提醒方式发送")
    var content = ""

    @Schema(title = "转接责任人(英文ID)，单人")
    var transferReceiver = ""

    @Schema(title = "重呼间隔（秒），默认为0")
    var interval = 0

    @Schema(title = "最大重呼次数，默认为0")
    var recallTime = 0
    ...
    data class TextNotify(
        @Schema(title = "是否开启企业微信提醒")
        val enabled: Boolean = false,
        @Schema(title = "提醒Title")
        val title: String = "",
        @Schema(title = "提醒内容")
        val content: String = ""
    ) {
