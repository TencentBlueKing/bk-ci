...
open class VoiceNotifyMessage : BaseMessage() {
    @Schema(description = "接收人(英文ID)，支持多个")
    var receivers = mutableSetOf<String>()

    @Schema(description = "任务名称，不超过200字符")
    var taskName = ""

    @Schema(description = "呼叫内容，建议只传简短的文字内容，详细信息通过企业微信提醒方式发送")
    var content = ""

    @Schema(description = "转接责任人(英文ID)，单人")
    var transferReceiver = ""

    @Schema(description = "重呼间隔（秒），默认为0")
    var interval = 0

    @Schema(description = "最大重呼次数，默认为0")
    var recallTime = 0

    @Schema(description = "企业微信提醒")
    var textNotify = TextNotify.DEFAULT

    @Schema(description = "语音信息--企业微信提醒")
    data class TextNotify(
        @Schema(description = "是否开启企业微信提醒")
        val enabled: Boolean = false,
        @Schema(description = "提醒Title")
        val title: String = "",
