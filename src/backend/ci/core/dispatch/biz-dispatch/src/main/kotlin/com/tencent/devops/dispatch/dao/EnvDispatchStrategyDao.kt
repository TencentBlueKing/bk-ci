...
import com.tencent.devops.model.dispatch.tables.TDispatchEnvStrategy
import com.tencent.devops.model.dispatch.tables.records.TDispatchEnvStrategyRecord
...
class EnvDispatchStrategyDao {
    ...
    fun create(
        ...
        with(TDispatchEnvStrategy.T_DISPATCH_ENV_STRATEGY) {
            return dslContext.insertInto(
                ...
                config.defaultStrategyCode?.name,
                config.strategyName,
                config.scope.name,
                config.nodeRule.name,
    ...
    fun batchCreate(
        dslContext: DSLContext,
        configs: List<DispatchStrategyConfig>
    ) {
        val now = LocalDateTime.now()
        with(TDispatchEnvStrategy.T_DISPATCH_ENV_STRATEGY) {
    ...
    fun listByEnv(
        ...
        with(TDispatchEnvStrategy.T_DISPATCH_ENV_STRATEGY) {
            return dslContext.selectFrom(this)
                ...
                .fetch(strategyMapper)
        }
    }

    fun batchUpdatePriority(
        dslContext: DSLContext,
        updates: List<Pair<Long, Int>>
    ) {
        val now = LocalDateTime.now()
        with(TDispatchEnvStrategy.T_DISPATCH_ENV_STRATEGY) {
    ...
    fun countCustomEnabled(
        ...
        with(TDispatchEnvStrategy.T_DISPATCH_ENV_STRATEGY) {
            return dslContext.selectCount().from(this)
                .where(PROJECT_ID.eq(projectId))
                .and(ENV_ID.eq(envId))
                .and(STRATEGY_TYPE.eq(StrategyType.CUSTOM.name))
                .and(ENABLED.eq(true))
    ...
    companion object {
        private val strategyMapper = DispatchStrategyRecordMapper()
    }
}

class DispatchStrategyRecordMapper :
    RecordMapper<TDispatchEnvStrategyRecord, DispatchStrategyConfig> {
    override fun map(record: TDispatchEnvStrategyRecord?): DispatchStrategyConfig? {
        return record?.let {
            DispatchStrategyConfig(
                id = it.id,
                ...
                defaultStrategyCode = it.defaultStrategyCode?.let { code ->
                    DefaultStrategyCode.valueOf(code)
                },
                ...
                labelSelector = it.labelSelector?.let { json ->
                    JsonUtil.to(json, object : com.fasterxml.jackson.core.type.TypeReference<List<LabelSelector>>() {})
                },
                enabled = it.enabled,
