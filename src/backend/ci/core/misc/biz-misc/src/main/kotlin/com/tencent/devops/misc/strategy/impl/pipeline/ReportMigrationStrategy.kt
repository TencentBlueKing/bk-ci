...
class ReportMigrationStrategy(
    ...
    private val logger = LoggerFactory.getLogger(ReportMigrationStrategy::class.java)

    override fun migrate(context: MigrationContext) {
        val pipelineId = context.pipelineId ?: run {
            logger.warn("Skipping T_REPORT migration: pipelineId is null")
            return
        }
        // 迁移T_REPORT表数据
        logger.info("Start migrating T_REPORT data for pipeline $pipelineId")
        ...
        logger.info("Finish migrating T_REPORT data for pipeline $pipelineId")
    }
}
