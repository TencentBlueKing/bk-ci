...
class DebugTimeoutScheduler @Autowired constructor(
    ...
    fun cleanTimeoutDebugVm() {
        ...
        timeoutRecords.forEach { record ->
            processTimeoutDebugRecord(record)
        }

        logger.info("Finished processing ${timeoutRecords.size} timeout debug records")
    }

    private fun processTimeoutDebugRecord(record: TDebugHistoryRecord) {
        try {
            logger.info(
            ...
            closeDebugLogin(record)
            deleteNewCreatedDebugVm(record)
            updateDebugRecordStatus(record)
        } catch (e: Exception) {
            logger.error(
    ...
    private fun closeDebugLogin(record: TDebugHistoryRecord) {
    ...
    private fun deleteNewCreatedDebugVm(record: TDebugHistoryRecord) {
        if (!record.newCreatedVm) return
    ...
    private fun updateDebugRecordStatus(record: TDebugHistoryRecord) {
