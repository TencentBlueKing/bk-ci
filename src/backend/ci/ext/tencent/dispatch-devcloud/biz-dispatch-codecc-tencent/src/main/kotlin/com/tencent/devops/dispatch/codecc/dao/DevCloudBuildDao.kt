...
import com.tencent.devops.model.dispatch.codecc.tables.records.TDevcloudBuildRecord
import com.tencent.devops.model.dispatch.codecc.tables.TDevcloudBuild
...
class DevCloudBuildDao {
    ...
    fun getNoUseIdleContainer(dslContext: DSLContext): Result<TDevcloudBuildRecord> {
        with(TDevcloudBuild.T_DEVCLOUD_BUILD) {
            return dslContext.selectFrom(this)
                    .where(timestampDiff(DatePart.DAY, UPDATE_TIME.cast(java.sql.Timestamp::class.java)).greaterOrEqual(30))
                    .and(STATUS.eq(0))
                    .fetch()
        }
    }
