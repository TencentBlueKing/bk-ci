...
-- 构建机准备相关
CREATE CONTINUOUS QUERY cq_dispatch_devcloud_total_count ON monitoring BEGIN SELECT count(errorCode) AS devcloud_total_count INTO monitoring.monitoring_retention.DispatchStatus_success_rat_count FROM monitoring.monitoring_retention.DispatchStatus WHERE buildType = '.devcloud.public' GROUP BY time(5m) END
CREATE CONTINUOUS QUERY cq_dispatch_devcloud_failed_count ON monitoring BEGIN SELECT count(errorCode) AS devcloud_failed_count INTO monitoring.monitoring_retention.DispatchStatus_success_rat_count FROM monitoring.monitoring_retention.DispatchStatus WHERE buildType = '.devcloud.public' AND errorCode != '0' GROUP BY time(5m) END
CREATE CONTINUOUS QUERY cq_dispatch_devcloud_success_count ON monitoring BEGIN SELECT count(errorCode) AS devcloud_success_count INTO monitoring.monitoring_retention.DispatchStatus_success_rat_count FROM monitoring.monitoring_retention.DispatchStatus WHERE buildType = '.devcloud.public' AND errorCode = '0' GROUP BY time(5m) END
