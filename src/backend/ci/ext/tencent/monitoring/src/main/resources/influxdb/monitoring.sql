...
-- 构建机准备相关
CREATE CONTINUOUS QUERY cq_dispatch_devcloud_total_count ON monitoring BEGIN SELECT count(errorCode) AS devcloud_total_count INTO monitoring.monitoring_retention.DispatchStatus_success_rat_count FROM monitoring.monitoring_retention.DispatchStatus  GROUP BY time(5m),buildType END
...
CREATE CONTINUOUS QUERY cq_dispatch_devcloud_start_count ON monitoring BEGIN SELECT count(errorCode) AS devcloud_start_count INTO monitoring.monitoring_retention.DispatchStatus_success_rat_count FROM monitoring.monitoring_retention.DispatchStatus WHERE actionType='start' AND errorCode='0' GROUP BY time(5m),buildType END
CREATE CONTINUOUS QUERY cq_dispatch_devcloud_stop_count ON monitoring BEGIN SELECT count(errorCode) AS devcloud_stop_count INTO monitoring.monitoring_retention.DispatchStatus_success_rat_count FROM monitoring.monitoring_retention.DispatchStatus WHERE actionType='stop' AND errorCode='0' GROUP BY time(5m),buildType END
