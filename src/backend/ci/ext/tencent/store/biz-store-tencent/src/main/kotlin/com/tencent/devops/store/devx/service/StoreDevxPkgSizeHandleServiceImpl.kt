...
class StoreDevxPkgSizeHandleServiceImpl : AbstractStoreComponentPkgSizeHandleService() {
    ...
    override fun batchUpdateComponentsVersionSize() {
        val redisLock = RedisLock(
            ...
            lockKey = "store:batch:pkg-size:${StoreTypeEnum.DEVX.name}",
     ...
     */
    private fun updateStorePackageSizeForStore(storeId: String, records: List<Record>) {
     ...
     */
    private fun buildPackageInfoFromRecord(record: Record): StorePackageInfoReq? {
        val pkgPath = record.get("PKG_PATH")?.toString()
        if (pkgPath.isNullOrBlank()) {
            logger.warn("buildPackageInfoFromRecord|pkgPath is null, skip record: $record")
            return null
        }
        // 远程调用归档服务获取文件实际大小
        ...
        if (nodeSize == null) {
            logger.warn("buildPackageInfoFromRecord|getFileSize returns null, pkgPath: $pkgPath")
            return null
        }
    ...
    override fun getComponentVersionSize(
        ...
        return StoreVersionSizeInfo(
            ...
            version = version,
            packageSize = size,
            unit = StoreConstants.MB_UNIT
        )
    }

    companion object {
        private val logger = LoggerFactory.getLogger(StoreDevxPkgSizeHandleServiceImpl::class.java)
    }
}