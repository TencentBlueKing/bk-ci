...
func process(f restful.RouteFunction, opts ProcessType) func(req *restful.Request, resp *restful.Response) {
	return func(req *restful.Request, resp *restful.Response) {
		...
		blog.Infof("Receive %s %s?%s From %s", req.Request.Method, req.Request.URL.Path, req.Request.URL.RawQuery, req.Request.RemoteAddr)
		...
		case ProcessMasterOnly:
			...
			if err != nil {
				...
				ReturnRest(&RestResponse{Resp: resp, ErrCode: commonTypes.ServerErrPreProcessFailed, HTTPCode: http.StatusInternalServerError})
			} else if isMaster {
				f(req, resp)
			} else {
		...
		blog.Infof("Return [%d] %dms %s %s To %s", resp.StatusCode(), useTime, req.Request.Method, req.Request.URL.Path, req.Request.RemoteAddr)
	}
}
...
func redirect(uri string, req *restful.Request, resp *restful.Response) {
	...
	if err != nil {
		...
		ReturnRest(&RestResponse{Resp: resp, ErrCode: commonTypes.ServerErrRedirectFailed, HTTPCode: http.StatusInternalServerError})
		return
	}
	r.Close = true
