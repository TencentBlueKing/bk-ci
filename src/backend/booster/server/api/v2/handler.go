...
func ApplyResource(req *restful.Request, resp *restful.Response) {
	...
	if err != nil {
		blog.Errorf("apply resource: get task info(%s) from engine(%s) failed, url(%s): %v", tb.ID, tb.Client.EngineName.String(), req.Request.URL.String(), err)
		api.ReturnRest(&api.RestResponse{Resp: resp, ErrCode: api.ServerErrRequestTaskInfoFailed, Message: err.Error()})
		return
	}

	blog.Infof("success to apply resource for task(%s) of project(%s) in engine(%s)", tb.ID, tb.Client.ProjectID, tb.Client.EngineName.String())
...
func SendMessage(req *restful.Request, resp *restful.Response) {
	...
	case MessageTask:
		if data, err = defaultManager.SendTaskMessage(param.TaskID, []byte(param.Extra)); err != nil {
			blog.Errorf("send message: send task(%s) message to engine failed, url(%s) message(%s): %v", param.TaskID, req.Request.URL.String(), param.Extra, err)
	...
	case MessageProject:
		if data, err = defaultManager.SendProjectMessage(param.ProjectID, []byte(param.Extra)); err != nil {
			blog.Errorf("send message: send project(%s) message to engine failed, url(%s) message(%s): %v", param.ProjectID, req.Request.URL.String(), param.Extra, err)
			api.ReturnRest(&api.RestResponse{Resp: resp, ErrCode: api.ServerErrSendMessageFailed, Message: err.Error()})
			return
		}
	default:
		blog.Errorf("send message: get unknown message type(%s) from param, url(%s) message(%s): %v", param.Type, req.Request.URL.String(), param.Extra, err)
...
func QueryTaskInfo(req *restful.Request, resp *restful.Response) {
	...
	if taskID == "" {
		...
		api.ReturnRest(&api.RestResponse{Resp: resp, ErrCode: api.ServerErrInvalidParam, Message: "task_id no specific"})
		return
	}

	info, err := getTaskInfo(taskID)
	if err != nil {
		blog.Errorf("apply resource: get task info(%s) failed, url(%s): %v", taskID, req.Request.URL.String(), err)
		api.ReturnRest(&api.RestResponse{Resp: resp, ErrCode: api.ServerErrRequestTaskInfoFailed, Message: err.Error()})
		return
	}
...
func UpdateHeartbeat(req *restful.Request, resp *restful.Response) {
	...
	if err = defaultManager.UpdateHeartbeat(taskID); err != nil {
		blog.Warnf("update heartbeat: update task(%s) heartbeat failed, url(%s): %v", taskID, req.Request.URL.String(), err)
...
func ReleaseResource(req *restful.Request, resp *restful.Response) {
	...
	if err = defaultManager.ReleaseTask(param); err != nil {
		blog.Errorf("release resource: release task(%s) failed, url(%s): %v", param.TaskID, req.Request.URL.String(), err)
...
func getApplyParam(req *restful.Request) (*manager.TaskCreateParam, error) {
	...
	blog.Infof("get apply param: get client status(project ID/client IP/user version: %s %s %s", param.ProjectID, param.ClientIP, param.ClientVersion)
	return param, nil
}
...
func getTaskInfo(taskID string) (*RespTaskInfo, error) {
	...
	return &RespTaskInfo{
		TaskID:      tb.ID,
		Status:      string(tb.Status.Status),
