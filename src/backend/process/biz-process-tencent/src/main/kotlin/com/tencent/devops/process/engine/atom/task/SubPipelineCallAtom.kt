...
class SubPipelineCallAtom @Autowired constructor(
    ...
    override fun tryFinish(task: PipelineBuildTask, param: SubPipelineCallElement, runVariables: Map<String, String>, force: Boolean): AtomResponse {
        ...
        return if (task.subBuildId == null || task.subBuildId.isNullOrBlank()) {
            ...
            return if (buildInfo == null) {
                LogUtils.addRedLine(rabbitTemplate, task.buildId, "找不到对应子流水线", task.taskId, task.executeCount ?: 1)
                AtomResponse(
                    buildStatus = BuildStatus.FAILED,
                    errorType = ErrorType.USER,
                    errorCode = AtomErrorCode.USER_RESOURCE_NOT_FOUND,
                ...
                when {
                    BuildStatus.isCancel(buildInfo.status) ->
                        LogUtils.addYellowLine(rabbitTemplate, task.buildId, "子流水线被取消", task.taskId, task.executeCount ?: 1)
                    BuildStatus.isFailure(buildInfo.status) ->
                        LogUtils.addYellowLine(rabbitTemplate, task.buildId, "子流水线执行失败", task.taskId, task.executeCount ?: 1)
                    BuildStatus.isSuccess(buildInfo.status) ->
                        LogUtils.addLine(rabbitTemplate, task.buildId, "子流水线执行成功", task.taskId, task.executeCount ?: 1)
                    else ->
                        return AtomResponse(
    ...
    override fun execute(task: PipelineBuildTask, param: SubPipelineCallElement, runVariables: Map<String, String>): AtomResponse {
        ...
        if (subPipelineId.isNullOrBlank())
            throw BuildTaskException(ERROR_BUILD_TASK_SUBPIPELINEID_NULL,
                    "子流水线ID参数为空:$subPipelineId，请检查流水线重新保存后并重新执行", task.pipelineId, buildId, taskId)

        // 注意：加projectId限定了不允许跨项目，防止恶意传递了跨项目的项目id
        val pipelineInfo = (pipelineRepositoryService.getPipelineInfo(projectId, subPipelineId!!)
                ?: throw BuildTaskException(ERROR_BUILD_TASK_SUBPIPELINEID_NOT_EXISTS,
        ...
        LogUtils.addLine(rabbitTemplate, buildId, "已启动子流水线 - ${pipelineInfo.pipelineName}", taskId, task.executeCount ?: 1)
