...
class BcsContainerOpByNameAtom @Autowired constructor(
    ...
    private fun doBcsOperation(
        ...
        if (param.bcsAppInstName.isNullOrBlank()) {
            ...
            LogUtils.addRedLine(rabbitTemplate, task.buildId, "bcsAppInstName is not init", task.taskId, task.executeCount ?: 1)
            return AtomResponse(
                buildStatus = BuildStatus.FAILED,
                errorType = ErrorType.USER,
                errorCode = USER_INPUT_INVAILD,
        ...
        if (param.namespace.isNullOrBlank()) {
            ...
            LogUtils.addRedLine(rabbitTemplate, task.buildId, "namespace is not init", task.taskId, task.executeCount ?: 1)
            return AtomResponse(
                buildStatus = BuildStatus.FAILED,
                errorType = ErrorType.USER,
                errorCode = USER_INPUT_INVAILD,
        ...
        when (opType) {
            ...
            BcsOperation.ROLLINGUPDATE -> {
                if (param.instVersionName.isNullOrBlank()) {
                    ...
                    LogUtils.addRedLine(rabbitTemplate, task.buildId, "instVersionName is not init", task.taskId, task.executeCount ?: 1)
                    return AtomResponse(
                        buildStatus = BuildStatus.FAILED,
                        errorType = ErrorType.USER,
                        errorCode = USER_INPUT_INVAILD,
            ...
            BcsOperation.SIGNAL -> {
                if (param.signal.isNullOrBlank()) {
                    ...
                    LogUtils.addRedLine(rabbitTemplate, task.buildId, "signal is not init", task.taskId, task.executeCount ?: 1)
                    return AtomResponse(
                        buildStatus = BuildStatus.FAILED,
                        errorType = ErrorType.USER,
                        errorCode = USER_INPUT_INVAILD,
                ...
                if (param.processName.isNullOrBlank()) {
                    ...
                    LogUtils.addRedLine(rabbitTemplate, task.buildId, "processName is not init", task.taskId, task.executeCount ?: 1)
                    return AtomResponse(
                        buildStatus = BuildStatus.FAILED,
                        errorType = ErrorType.USER,
                        errorCode = USER_INPUT_INVAILD,
            ...
            BcsOperation.COMMAND -> {
                if (param.command.isNullOrBlank()) {
                    ...
                    LogUtils.addRedLine(rabbitTemplate, task.buildId, "BCS command is not init", task.taskId, task.executeCount ?: 1)
                ...
                LogUtils.addLine(
                        ...
                        "BCS opType is command,command : $command", task.taskId, task.executeCount ?: 1
                )
    ...
    private fun createInstance(
        ...
        if (param.namespaceVar == null) {
            ...
            LogUtils.addRedLine(rabbitTemplate, task.buildId, "命名空间没有被赋值", task.taskId, task.executeCount ?: 1)
            return AtomResponse(
                buildStatus = BuildStatus.FAILED,
                errorType = ErrorType.USER,
                errorCode = USER_INPUT_INVAILD,
        ...
        if (param.showVersionName.isNullOrBlank()) {
            ...
            LogUtils.addRedLine(rabbitTemplate, task.buildId, "showVersionName is not init", task.taskId, task.executeCount ?: 1)
            return AtomResponse(
                buildStatus = BuildStatus.FAILED,
                errorType = ErrorType.USER,
                errorCode = USER_INPUT_INVAILD,
        ...
        OkhttpUtils.doHttp(request).use { response ->
            ...
            if (!response.isSuccessful) {
                ...
                LogUtils.addRedLine(rabbitTemplate, task.buildId, "创建实例失败，详情： $data", task.taskId, task.executeCount ?: 1)
                return AtomResponse(
                    buildStatus = BuildStatus.FAILED,
                    errorType = ErrorType.USER,
                    errorCode = USER_TASK_OPERATE_FAIL,
    ...
    private fun getBcsAppInstIdByName(
        ...
        OkhttpUtils.doHttp(request).use { response ->
            ...
            if (dataMap["id"] == null || dataMap["id"] == "") {
                logger.error("Get bcsAppInstId by bcsAppInstName($bcsAppInstName) failed, msg:$data")
                LogUtils.addRedLine(rabbitTemplate, task.buildId, "实例名称或命名空间错误", task.taskId, task.executeCount ?: 1)
