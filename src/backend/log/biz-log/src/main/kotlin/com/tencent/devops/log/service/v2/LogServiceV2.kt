...
class LogServiceV2 @Autowired constructor(
    ...
    fun addLogEvent(event: LogEvent) {
        ...
        try {
            ...
            logger.info("[${event.buildId}] It took ${System.currentTimeMillis() - currentEpoch}ms to add event log with result $success")
        }
    }
    ...
    fun addBatchLogEvent(event: LogBatchEvent) {
        ...
        try {
            ...
            logger.info("It took ${elapse}ms to add the logs ${event.logs.size} with result $success [${event.buildId}]")
    ...
    fun updateLogStatus(event: LogStatusEvent) {
        ...
        logger.info("[$buildId|$tag|$executeCount|$finished] Start to update log status")
        indexServiceV2.finish(buildId, tag, executeCount, finished)
    }
    ...
    fun queryMoreLogsBetweenLines(
        ...
        try {
            ...
            val queryLogs = QueryLogs(buildId, getLogStatus(buildId, tag, executeCount))

            try {
                val query = getQuery(buildId, tag, executeCount)
    ...
    fun queryMoreLogsAfterLine(
        ...
        try {
            ...
            val result = doQueryByKeywords(buildId, index, type, start, keywords, tag, executeCount)
            success = true
            return result
        } finally {
            logBeanV2.query(System.currentTimeMillis() - startEpoch, success)
    ...
    fun getEndLogs(pipelineId: String, buildId: String, tag: String?, executeCount: Int?, size: Int): EndPageQueryLogs {
        ...
        try {
            ...
            val pageLog = if (keywordsStr == null || keywordsStr.isBlank()) {
                if (isAnalysis) {
                    ...
                    pageResult = queryInitLogsPage(buildId, tag, executeCount, page, pageSize)
                    val logSize = getLogSize(index, type, buildId, tag, executeCount)
    ...
    private fun queryInitLogsPage(
        ...
        try {
            val logs = getLogsByPage(buildId, index, type, tag, executeCount, page, pageSize)
            queryLogs.logs.addAll(logs)
    ...
    private fun doQueryMoreLogsAfterLine(
        ...
        val moreLogs = QueryLogs(buildId, getLogStatus(buildId, tag, executeCount))
        ...
        try {
            ...
            if (wholeQuery) {
                val startQuery = getQuery(buildId, tag, executeCount)
                ...
                val stopQuery = getQuery(buildId, tag, executeCount)
...
//            }
    ...
    private fun doQueryByKeywords(
        ...
        val logStatus = getLogStatus(buildId, tag, executeCount)
        val initLogs = QueryLogs(buildId, logStatus)
        try {
            val size = getLogSize(index, type, buildId, tag, executeCount)
            if (size == 0L) {
                return initLogs
            }

            val logs = getLogs(buildId, index, type, keywords, false, tag, executeCount)
    ...
    private fun getLogSize(index: String, type: String, buildId: String, tag: String?, executeCount: Int?): Long {
        val query = getQuery(buildId, tag, executeCount)
        logger.info("[$index|$type|$buildId|$tag|$executeCount] Get the log size - ($query)")
        val searchResponse = client.prepareSearch(index)
            .setTypes(type)
            .setQuery(query)
            .setSize(0)
    ...
    private fun getQuery(buildId: String, tag: String?, executeCount: Int?): BoolQueryBuilder {
    ...
    private fun doQueryInitLogs(
        ...
        try {
            val logs = getLogs(buildId, index, type, keywords, wholeQuery, tag, executeCount)
            queryLogs.logs.addAll(logs)
            if (logs.isEmpty()) queryLogs.status = LogStatus.EMPTY
        } catch (ex: org.elasticsearch.index.IndexNotFoundException) {
            logger.error("Query init logs failed because of IndexNotFoundException. buildId: $buildId", ex)
    ...
    private fun getLogStatus(buildId: String, tag: String?, executeCount: Int?): Boolean {
        return indexServiceV2.isFinish(buildId, tag, executeCount)
    }
    ...
    private fun getLogs(
        ...
        logger.info("[$buildId|$index|$type|$tag|$executeCount] log params for type($type): index: $index, keywords: $keywords, wholeQuery: $wholeQuery, tag: $tag, executeCount: $executeCount")
        ...
        val logRange =
            if (tag.isNullOrBlank()) Pair(1L, size) else getLogRange(buildId, index, type, tag!!, executeCount, size)
        ...
        if (!lineRanges.isEmpty()) {
            val boolQueryBuilder = getQuery(buildId, tag, executeCount)
    ...
    private fun getLogRange(
        ...
        val q = getQuery(buildId, tag, executeCount)
            .must(
        ...
        return getRangeIndex(hits, tag)
    }

    private fun getRangeIndex(hits: SearchHits, tag: String): Pair<Long, Long> {
        var beginIndex: Long? = null
        var endIndex: Long? = null
        run lit@{
            hits.forEach { hit ->
                if (hit.source["tag"] == tag) {
                    if (hit.source["logType"] == LogType.START.name) {
                        beginIndex = hit.source["lineNo"].toString().toLong()
                    } else if (hit.source["logType"] == LogType.END.name) {
                        endIndex = hit.source["lineNo"].toString().toLong()
                    }
